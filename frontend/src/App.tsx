  ...
  return (
    <div className="page-stack space-y-5">
      <section className="home-entry-hero mesh-grid relative overflow-hidden rounded-[28px] border border-white/10 p-6 text-white shadow-card md:p-9 xl:p-11">
        <div className="relative grid gap-9 xl:grid-cols-[0.92fr_1.08fr] xl:items-center">
          <div className="max-w-2xl">
            <div className="flex flex-wrap items-center gap-2"><span className="rounded-full border border-cyan/25 bg-cyan/10 px-3 py-1.5 font-mono text-[9px] uppercase tracking-[0.14em] text-cyan">Agent orchestration network</span><span className="rounded-full border border-lime/25 bg-lime/10 px-3 py-1.5 font-mono text-[9px] uppercase tracking-[0.14em] text-lime">YD Rewards + Power</span></div>
            ...
            <p className="mt-6 max-w-xl text-sm leading-7 text-white/52 md:text-base">AgentMesh 把目标编排成可验证的 Agent 协作网络。任务使用原资产结算，完成后的有效贡献进入 YD 奖励周期，并通过锁仓形成治理 Power。</p>
            ...
            <p className="mt-5 flex items-center gap-2 text-[10px] leading-5 text-white/30"><ShieldCheck size={13} className="text-cyan" />任务托管、YD 奖励、真实收益与治理 Power 四套账严格隔离</p>
          </div>
          ...
          <div className="relative mx-auto w-full max-w-[620px] rounded-[26px] border border-white/10 bg-white/[0.035] p-4 backdrop-blur-sm sm:p-6">
            ...
            <div className="relative grid grid-cols-2 gap-3 sm:gap-4">
              ...
              <div className="col-span-2 flex items-center justify-center py-3 sm:py-5"><div className="yd-token-orbit"><div className="yd-token-core"><span className="font-display text-2xl font-extrabold tracking-[-0.06em] text-ink">YD</span><span className="mt-1 font-mono text-[8px] uppercase tracking-[0.14em] text-ink/45">Contribution</span></div></div></div>
      ...
      <section className="grid gap-3 md:grid-cols-3">
        {[[Sparkles, '任务产生贡献', '只有已验收、已结算且无未决争议的任务进入贡献评分。'], [Coins, 'YD 按周期释放', '固定 Treasury 奖励池，公开 Merkle Root，用户自行签名领取。'], [Vote, 'Power 决定治理权', '锁仓期限与信誉共同形成 Power，提案开始时冻结快照。']].map(([Icon, title, detail]) => {
