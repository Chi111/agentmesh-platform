...
export function SettingsPage() {
  ...
  return (
    <div className="space-y-7">
      <PageHeader eyebrow="Workspace Settings" title="设置" description="管理身份、通知、钱包与工作区偏好。真实设置由 AgentMesh Worker 鉴权并持久化到 D1。" />
      ...
      <div className="grid gap-5 xl:grid-cols-[240px_minmax(0,1fr)]">
        ...
        <section className="panel p-5 md:p-6" aria-live="polite">
          ...
          {!loading && section === 'profile' ? <><div className="border-b border-line pb-4"><h2 className="font-semibold">个人资料</h2><p className="mt-1 text-xs text-muted">身份字段由已验证的登录账户提供，避免浏览器伪造。</p></div><div className="mt-2"><FactRow label="显示名称" value={profile?.displayName ?? '未命名账户'} /><FactRow label="邮箱" value={profile?.email ?? '未关联邮箱'} /><FactRow label="平台角色" value={profile?.role === 'admin' ? '平台管理员' : profile?.role === 'developer' ? 'Agent 开发者' : '任务方'} /><FactRow label="身份提供方" value={provider === 'privy' ? 'Privy · Web2 / Web3' : 'PinMe Identity · Web2'} /><FactRow label="Profile ID" value={profile?.id ?? '正在同步'} mono /></div></> : null}
          ...
          {!loading && section === 'wallet' ? <><div className="border-b border-line pb-4"><h2 className="font-semibold">钱包与结算</h2><p className="mt-1 text-xs text-muted">钱包由认证提供方管理；AgentMesh 不接收或托管私钥。</p></div><div className="mt-5 grid gap-4 md:grid-cols-2"><article className="rounded-2xl border border-line bg-canvas p-4"><WalletCards size={19} className="text-cyan" /><p className="mt-4 text-sm font-semibold">{walletAddress ? '已关联 Ethereum 钱包' : '尚未关联外部钱包'}</p><p className="mt-2 break-all font-mono text-[10px] leading-5 text-muted">{walletAddress ?? '启用 Privy 后可通过签名连接外部钱包或创建嵌入式钱包。'}</p>{profile && provider === 'privy' && !walletAddress ? <button type="button" className="btn-secondary mt-4" onClick={() => void linkWallet()}>关联钱包</button> : null}</article><article className="rounded-2xl border border-line bg-canvas p-4"><Server size={19} className="text-cyan" /><p className="mt-4 text-sm font-semibold">CREDIT / mUSDC / sETH</p><p className="mt-2 text-xs leading-5 text-muted">Web2 从站内充值余额扣款；Web3 使用 Sepolia mUSDC 或原生 sETH，并由 Worker 校验托管事件。</p><span className="mono-chip mt-4">NO KEY CUSTODY</span></article></div></> : null}
