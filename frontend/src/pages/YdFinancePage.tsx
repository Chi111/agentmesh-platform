...
export function YdFinancePage() {
  ...
  const load = async () => {
    ...
    try {
      ...
      setError(loadError instanceof Error ? loadError.message : `${BRAND.contribution.centerLabel}加载失败。`);
    } finally {
      setLoading(false);
    }
  };
  ...
  const claim = async (epoch: RewardEpoch, allocation: (typeof allocations)[number]['allocation']) => {
    await run(`claim:${epoch.id}`, async () => {
      ...
      return txHash;
    }, `${BRAND.contribution.symbol} 奖励已领取并完成链上核验`);
  };
  ...
  const changeStake = async (event: FormEvent) => {
    ...
    await run('stake', async () => {
      ...
      return txHash;
    }, hasStake ? '锁仓数量已增加' : `${BRAND.contribution.symbol} 已锁定并生成 ${BRAND.contribution.powerName}`);
  };
  ...
  return (
    <div className="space-y-7">
      <PageHeader
        eyebrow={BRAND.contribution.eyebrow}
        title={BRAND.contribution.displayName}
        description={`任务结算、${BRAND.contribution.symbol} 奖励与治理 ${BRAND.contribution.powerName} 独立记账。这里不接触任务托管资金，也不提供真实收益或 APY；${BRAND.contribution.disclaimer}`}
      ...
      <section className="mesh-grid overflow-hidden rounded-2xl bg-ink p-5 text-white sm:p-7">
        <div className="flex flex-wrap items-start justify-between gap-5">
          <div className="max-w-2xl"><p className="flex items-center gap-2 text-xs font-semibold text-lime"><ShieldAlert size={16} />四套账严格隔离</p><h2 className="mt-3 text-xl font-semibold">{BRAND.contribution.symbol} 是 {BRAND.platform.name} 的{BRAND.contribution.purposeLabel}，不是任务支付资产</h2><p className="mt-2 text-sm leading-6 text-white/50">任务贡献须已验收、已结算且无未决争议；仲裁履职审核通过后按固定报酬记账，无需等待案件资金执行。{BRAND.contribution.shortName} 名称关联 {BRAND.evidence.providerName} 内容证据能力，但不代表 {BRAND.evidence.providerName} 官方发行或背书。Earn Vault 尚未启用，页面不会展示虚构 APY。</p></div>
          <div className="grid min-w-[280px] grid-cols-2 gap-3"><div className="rounded-xl border border-white/10 bg-white/5 p-4"><p className="text-[9px] uppercase tracking-wider text-white/30">{BRAND.contribution.symbol} Balance</p><p className="mt-2 font-mono text-lg">{formatYdUnits(walletBalance)} {BRAND.contribution.symbol}</p></div><div className="rounded-xl border border-white/10 bg-white/5 p-4"><p className="text-[9px] uppercase tracking-wider text-white/30">Governance {BRAND.contribution.powerName}</p><p className="mt-2 font-mono text-lg">{formatPower(staking?.votingPower ?? '0')}</p></div></div>
        </div>
      ...
      {!overview?.config.configured || !ydWalletEnabled ? <section className="rounded-2xl border border-warning/30 bg-warning/10 p-5"><p className="font-semibold">{BRAND.contribution.symbol} 测试网合约尚未完整配置</p><p className="mt-2 text-sm leading-6 text-muted">当前仅更新未来测试代币 metadata；没有部署合约，也没有迁移旧 {BRAND.contribution.symbol} 地址、余额或历史交易。在地址写入 Worker Secret / 前端公开配置前禁止链上操作。</p></section> : !identityWallet ? <section className="rounded-2xl border border-cyan/25 bg-cyan/[0.06] p-5"><p className="font-semibold">连接认证钱包后操作 {BRAND.contribution.symbol}</p><p className="mt-2 text-sm text-muted">领取、锁仓和委托都必须由 {BRAND.platform.name} 已绑定的钱包签名。</p><button type="button" className="btn-primary mt-4" onClick={() => void linkWallet()}><Landmark size={16} />连接钱包</button></section> : null}
      ...
      <div className="grid gap-6 2xl:grid-cols-[minmax(0,1.1fr)_minmax(360px,.9fr)]">
        <section className="panel p-5 sm:p-6">
          ...
          <div className="mt-5 space-y-3">{allocations.length ? allocations.map(({ allocation, epoch }) => <article className="rounded-2xl border border-line p-4" key={allocation.id}><div className="flex flex-wrap items-start justify-between gap-3"><div><p className="text-xs text-muted">Epoch {epoch?.epochNumber ?? '—'} · {epoch?.status ?? 'unknown'}</p><p className="mt-2 font-mono text-2xl font-semibold">{formatYdUnits(allocation.amountUnits)} <span className="text-sm text-cyan">{BRAND.contribution.symbol}</span></p><p className="mt-2 text-xs text-muted">领取截止 {formatDate(epoch?.claimEndsAt ?? null)}</p></div>{allocation.status === 'claimed' ? <StatusBadge tone="success">已领取</StatusBadge> : epoch?.status === 'published' ? <button type="button" className="btn-primary" disabled={Boolean(busy) || !ydWalletEnabled} onClick={() => void claim(epoch, allocation)}>{busy === `claim:${epoch.id}` ? <LoaderCircle className="animate-spin" size={15} /> : <Sparkles size={15} />}领取 {BRAND.contribution.symbol}</button> : <StatusBadge tone="neutral">等待发布</StatusBadge>}</div></article>) : <div className="rounded-2xl border border-dashed border-line bg-canvas/40 p-8 text-center"><Gift className="mx-auto text-cyan" size={24} /><p className="mt-4 text-sm font-semibold">暂无周期分配</p><p className="mt-2 text-xs leading-5 text-muted">任务结算或仲裁履职审核通过后进入周期分配；领取须等待周期发布。</p></div>}</div>
          <div className="mt-6 border-t border-line pt-5"><div className="flex items-center gap-2"><History size={16} /><h3 className="text-sm font-semibold">贡献来源</h3></div>{overview?.activities.length ? <div className="mt-3 divide-y divide-line">{overview.activities.slice(0, 12).map((activity) => <div className="flex items-center justify-between gap-4 py-3" key={activity.id}><div><p className="text-xs font-semibold">{roleLabel[activity.role]} · {activity.missionId ?? activity.disputeId ?? '平台治理'}</p><p className="mt-1 text-[10px] text-muted">{activity.detail?.source === 'reviewed_arbitration_work' ? '履职审核通过 · 固定报酬，未分配权益自动结转' : `${activity.asset} ${activity.settledAmount.toLocaleString()} · 质量 ${activity.qualityBps / 100}% · 惩罚 ${activity.penaltyBps / 100}%`}</p></div><span className="font-mono text-xs text-cyan">{activity.detail?.source === 'reviewed_arbitration_work' ? `${formatYdUnits(String(activity.detail.fixedRewardUnits))} ${BRAND.contribution.symbol}` : `${activity.scoreMicros.toLocaleString()} pts`}</span></div>)}</div> : <p className="mt-3 text-xs text-muted">当前账户还没有符合条件的任务贡献或已审核仲裁报酬。</p>}</div>
        </section>

        <section className="panel p-5 sm:p-6">
          <div className="flex items-center gap-3 border-b border-line pb-5"><span className="flex size-10 items-center justify-center rounded-xl bg-lime/15"><LockKeyhole size={18} /></span><div><h2 className="font-semibold">锁仓与 {BRAND.contribution.powerName}</h2><p className="mt-1 text-xs text-muted">非转让 {BRAND.contribution.powerName} · 30–730 天 · 历史区块快照</p></div></div>
          <div className="mt-5 grid grid-cols-2 gap-3"><div className="rounded-xl bg-canvas p-4"><p className="text-[10px] text-muted">已锁定</p><p className="mt-2 font-mono text-lg">{formatYdUnits(staking?.amountUnits ?? '0')} {BRAND.contribution.symbol}</p></div><div className="rounded-xl bg-canvas p-4"><p className="text-[10px] text-muted">信誉系数</p><p className="mt-2 font-mono text-lg">{((staking?.reputationBps ?? 10_000) / 10_000).toFixed(2)}×</p></div></div>
          ...
          <div className="grid gap-6 border-t border-line p-5 xl:grid-cols-2 sm:p-6">
            <div>
              ...
              <label className="mt-4 block">
                <span className="field-label">{BRAND.platform.name} 绑定钱包</span>
            ...
            <div>
              ...
              <p className="mt-2 text-xs leading-5 text-muted">仅领取窗口已经结束的已发布周期可清扫；合约会把未领取 {BRAND.contribution.symbol} 退回 Treasury。</p>
