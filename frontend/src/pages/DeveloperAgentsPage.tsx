        ...
        {ownedAgents.map((agent) => (
          <article className="panel p-5" key={agent.id}>
            <div className="grid gap-5 lg:grid-cols-[minmax(0,1fr)_repeat(3,120px)_auto] lg:items-center">
              <div className="flex min-w-0 items-center gap-4"><AgentAvatar agent={agent} size="lg" /><div className="min-w-0"><div className="flex flex-wrap items-center gap-2"><h2 className="truncate text-lg font-semibold">{agent.name}</h2><StatusBadge tone={agent.status === 'active' ? 'success' : 'warning'}>{agent.status === 'active' ? '接单中' : '试炼中'}</StatusBadge></div><p className="mt-1 font-mono text-[10px] text-muted">{agent.version} · HTTPS / JSON · {agent.category}</p><p className="mt-2 line-clamp-1 text-xs text-muted">{agent.summary}</p></div></div>
              <div><p className="text-[10px] text-muted">成功率</p><p className="mt-1 font-mono text-base font-semibold">{agent.successRate || '—'}{agent.successRate ? '%' : ''}</p></div>
              <div><p className="text-[10px] text-muted">响应时间</p><p className="mt-1 font-mono text-base font-semibold">{agent.responseTime}</p></div>
