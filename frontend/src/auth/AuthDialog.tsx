...
export function AuthDialog({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  if (provider === 'privy') {
    return (
      <Modal open={open} onClose={onClose} title={`进入 ${BRAND.platform.name}`} description={`一个账户同时覆盖 Web2 与 Web3。邮箱${privyGoogleEnabled ? '、Google' : ''}和钱包身份最终进入同一个真实工作区。`}>
        <div className="overflow-hidden rounded-2xl border border-line bg-canvas">
          <div className="border-b border-line bg-ink px-5 py-5 text-white">
            ...
            <p className="mt-4 max-w-sm text-sm leading-6 text-white/65">登录方式只是入口，任务归属和操作权限仍由 {BRAND.platform.name} Worker 校验，不把钱包地址当作权限凭据。</p>
          </div>
  ...
  return (
    <Modal open={open} onClose={onClose} title={mode === 'login' ? '连接正式工作区' : `创建 ${BRAND.platform.name} 账户`} description={`登录后任务、Agent、通知和结算状态将写入 ${BRAND.evidence.providerName} Worker + D1；未登录仅可浏览公开 Agent 目录。`}>
