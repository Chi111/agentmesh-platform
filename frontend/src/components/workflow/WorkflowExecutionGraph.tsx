...
import type { Agent, Deliverable, ExecutionEvent, WorkflowEdge, WorkflowStage } from '../../types/domain';
import {
...
export function WorkflowExecutionGraph({ stages, edges, agents, events, deliverables, requester, busy, onApprove, onReject, onRetry }: Props) {
  ...
  const nodeDeliverables = selected ? deliverables.filter((deliverable) => deliverable.stageId === selected.id) : [];
  const selectedInput = selected?.input ?? {};
  const canRetry = requester && selected?.nodeType === 'task' && selected.status === 'failed';
  const canDecideGate = requester && selected?.nodeType === 'approval' && selected.status === 'running';
  ...
  return (
    <div className="overflow-hidden rounded-2xl border border-white/10 bg-ink text-white xl:flex xl:h-full xl:min-h-0 xl:flex-col">
      ...
      {selected ? <div className="flex max-h-[min(72dvh,640px)] min-h-0 flex-col border-t border-white/10 bg-black/20 xl:max-h-[44%] xl:shrink-0">
        <div className="flex shrink-0 flex-wrap items-center justify-between gap-3 border-b border-white/10 bg-[#0b1116] px-4 py-3">
          ...
          <div className="flex flex-wrap items-center justify-end gap-2">
            ...
            {canDecideGate ? <><button type="button" className="btn-signal !min-h-9 !px-3 !text-xs" disabled={busy} onClick={() => void onApprove(selected.id, feedback)}>{busy ? <LoaderCircle size={14} className="animate-spin" /> : <CheckCircle2 size={14} />}批准</button><button type="button" className="inline-flex min-h-9 items-center gap-2 rounded-xl bg-danger px-3 text-xs font-semibold text-white disabled:opacity-40" disabled={busy || !reworkIds.length || feedback.trim().length < 2} onClick={() => void onReject(selected.id, feedback, reworkIds)}>{busy ? <LoaderCircle size={14} className="animate-spin" /> : <XCircle size={14} />}驳回并返工</button></> : null}
          </div>
        </div>
        <div className="min-h-0 flex-1 overflow-y-auto overscroll-contain p-4 [scrollbar-color:rgba(255,255,255,0.2)_transparent] [scrollbar-width:thin]">
          {canDecideGate ? <div className="mb-4 rounded-xl border border-amber-300/20 bg-amber-300/5 p-4"><div className="flex items-center gap-2 text-sm font-semibold"><ShieldCheck size={16} className="text-amber-300" />人工审批 Gate</div><p className="mt-1 text-[10px] leading-4 text-white/40">可直接批准；如需驳回，请填写反馈并选择至少一个直接上游节点返工。</p><textarea className="mt-3 w-full rounded-xl border border-white/10 bg-white/5 p-3 text-sm text-white outline-none placeholder:text-white/25 focus:border-cyan/50" value={feedback} onChange={(event) => setFeedback(event.target.value)} placeholder="审批反馈（驳回时建议填写）" />
            <div className="mt-3 flex flex-wrap gap-2">{upstream.filter((stage) => stage.nodeType === 'task').map((stage) => <label className="inline-flex cursor-pointer items-center gap-2 rounded-lg border border-white/10 px-3 py-2 text-xs transition hover:border-cyan/35" key={stage.id}><input type="checkbox" checked={reworkIds.includes(stage.id)} onChange={(event) => setReworkIds((items) => event.target.checked ? [...items, stage.id] : items.filter((id) => id !== stage.id))} />返工：{stage.name}</label>)}</div>
          </div> : null}
