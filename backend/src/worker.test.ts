...
let injectedLlmContent: string;
let ipfsResponder: (() => Response | Promise<Response>) | null;
let publishedBundles: Array<{ directoryName: string; paths: string[] }>;
let publishedPinmeAppKeys: Array<string | undefined>;
...
async function createFundedMission(): Promise<{ mission: Mission; stages: WorkflowStage[] }> {
...
async function completePausedWorkflow(missionId: string, stages: WorkflowStage[]) {
  ...
  for (const stage of stages) {
    await store.updateStage(missionId, stage.id, 'done', stage.nodeType === 'approval'
      ? { decision: 'approved', summary: `${stage.name} 已批准` }
      : { summary: `${stage.name} 已完成`, verified: true });
    if (stage.nodeType !== 'task' || stage.input.executionMode !== 'implement') continue;
    const deliverable: Deliverable = {
      id: `DEL-${stage.id}-completed`, missionId, stageId: stage.id, agentId: stage.agentId,
      attemptNo: stage.attemptNo,
      name: `${stage.name} 交付物`, uri: `ipfs://bafy${stage.id.replaceAll('-', '').toLocaleLowerCase()}`,
      contentHash: `sha256:${'c'.repeat(64)}`, mimeType: 'application/zip', status: 'submitted', createdAt: completedAt,
    };
...
beforeEach(() => {
  ...
  app = createApp({
    ...
    llmCaller: async (_env, messages, options) => {
      requestedLlmModels.push(options?.model);
      const userMessage = messages.find((message) => message.role === 'user')?.content;
      if (userMessage) {
        try { requestedLlmInputs.push(JSON.parse(userMessage) as JsonBody); } catch { /* Some tests intentionally use plain text. */ }
      }
      return { content: injectedLlmContent };
    },
    pinmeUploader: async (_env, input) => {
      publishedPinmeAppKeys.push(_env.PINME_UPLOAD_APP_KEY);
      publishedBundles.push({ directoryName: input.directoryName, paths: input.files.map((file) => file.path) });
      return {
        rootCid: 'bafybeigdyrzt5sfp7udm7hu76uh7y26nf3bn5o2viwzgr4c6a3b6pydf4y',
        publicUrl: 'https://delivery-test.pinme.dev',
        shortUrl: 'https://short-test.pinme.dev',
        pinmeUrl: 'https://delivery-test.pinme.dev',
        dnsUrl: null,
      };
    },
...
describe('AgentMesh Worker', () => {
  ...
  it('uses a signed Privy identity token to map linked accounts to one canonical profile', async () => {
    ...
    const identityToken = await new SignJWT({
      linked_accounts: JSON.stringify([
        ...
        {
          type: 'wallet',
          address: '0x7100000000000000000000000000000000008F2C',
          chain_type: 'ethereum',
    ...
    expect(verified.identity?.walletAddressAuthoritative).toBe(true);
    ...
    const privyProfile = await store.ensureIdentityProfile({
      ...
      walletAddress: verified.identity?.walletAddress,
      walletAddressAuthoritative: verified.identity?.walletAddressAuthoritative,
      displayName: verified.identity?.displayName ?? 'Same User',
    });
  ...
  it('keeps Privy embedded wallets out of Web3 identity and removes a previously synchronized address', async () => {
    const subject = 'did:privy:web2-google-user';
    const embeddedWallet = '0x7200000000000000000000000000000000008F2C';
    await store.ensureIdentityProfile({
      provider: 'privy',
      subject,
      email: 'web2-only@example.com',
      displayName: 'Web2 Only',
    ...
    const identityToken = await new SignJWT({
      linked_accounts: JSON.stringify([
        { type: 'email', address: 'web2-only@example.com' },
        {
          type: 'wallet',
      ...
      .setExpirationTime('5m')
      .sign(privateKey);
    const verified = await verifyPrivyIdentityToken({
      PRIVY_APP_ID: 'privy-app-test',
      PRIVY_VERIFICATION_KEY: await exportSPKI(publicKey),
    }, identityToken, subject);

    expect(verified.identity?.walletAddress).toBeUndefined();
    expect(verified.identity?.walletAddressAuthoritative).toBe(true);
    const profile = await store.ensureIdentityProfile({
      provider: 'privy',
      subject,
      email: verified.identity?.email,
      displayName: verified.identity?.displayName ?? 'Web2 Only',
      walletAddress: verified.identity?.walletAddress,
      walletAddressAuthoritative: verified.identity?.walletAddressAuthoritative,
    });
    expect(profile.walletAddress).toBeUndefined();
  });
  ...
  it('publishes health and capability metadata without authentication', async () => {
    ...
    expect(capabilities.body.data.auth).not.toContain('embedded_wallet');
  ...
  it('defaults CORS to the production domains instead of a wildcard', async () => {
    ...
    const preflight = await app.fetch(new Request('http://local.test/api/health', {
      method: 'OPTIONS',
      headers: {
        Origin: 'https://5ed6cec3.pinit.eth.limo',
    ...
    expect(preflight.headers.get('Access-Control-Allow-Origin')).toBe('https://5ed6cec3.pinit.eth.limo');
  ...
  it('lets each user configure an encrypted PinMe AppKey without returning the secret', async () => {
    const initial = await api('/api/me/integrations/pinme', {}, requester.id);
    expect(initial.response.status).toBe(200);
    expect(initial.body.data).toEqual({ configured: false, addressHint: null, updatedAt: null });

    const invalid = await api('/api/me/integrations/pinme', {
      method: 'PUT', body: JSON.stringify({ appKey: 'not-an-app-key' }),
    }, requester.id);
    expect(invalid.response.status).toBe(400);
    expect(invalid.body.error.code).toBe('PINME_APP_KEY_INVALID');

    const appKey = '0x1234567890abcdef-user-owned-token';
    const saved = await api('/api/me/integrations/pinme', {
    ...
    expect(JSON.stringify(saved.body)).not.toContain('user-owned-token');
    const stored = await store.getUserPinmeCredential(requester.id);
    expect(stored?.ciphertext).not.toContain('user-owned-token');
    expect((await api('/api/me/integrations/pinme', {}, requester.id)).body.data).toEqual(saved.body.data);
    expect((await api('/api/me/integrations/pinme', {}, otherDeveloper.id)).body.data.configured).toBe(false);

    const removed = await api('/api/me/integrations/pinme', { method: 'DELETE' }, requester.id);
    expect(removed.response.status).toBe(200);
    expect(removed.body.data.configured).toBe(false);
    expect(await store.getUserPinmeCredential(requester.id)).toBeNull();
  });
  ...
  it('runs disputes through admin review and an append-only action trail', async () => {
    ...
    const prematureQueue = await api(`/api/disputes/${disputeId}/execution`, { method: 'POST', body: '{}' }, admin.id);
    expect(prematureQueue.response.status).toBe(409);
    expect(prematureQueue.body.error.code).toBe('ARBITRATION_EXECUTION_NOT_READY');
    currentNow = '2026-08-20T00:00:00.000Z';
    const queued = await api(`/api/disputes/${disputeId}/execution`, { method: 'POST', body: '{}' }, admin.id);
    expect(queued.response.status).toBe(201);
    expect(queued.body.data.execution).toMatchObject({ action: 'refund_requester', status: 'queued' });
    ...
    expect(actions.body.data.map((item: JsonBody) => item.action)).toEqual([
      'review_started', 'execution_queued', 'resolved', 'execution_executed',
    ]);
  ...
  it('lets only administrators manage the arbitration council', async () => {
    ...
    const appointed = await api(`/api/arbitration/members/${otherDeveloper.id}`, {
      method: 'PUT', body: JSON.stringify({ active: true, power: 7 }),
    }, admin.id);
    expect(appointed.response.status).toBe(200);
    expect(appointed.body.data).toMatchObject({ userId: otherDeveloper.id, status: 'active', power: 7 });
  ...
  it('freezes Power voting, preserves the first round, and allows exactly one expanded-council appeal', async () => {
    const low = { id: 'arb-low', email: 'low@example.test', displayName: 'Low Power', role: 'requester' as const };
    const high = { id: 'arb-high', email: 'high@example.test', displayName: 'High Power', role: 'requester' as const };
    const extra = { id: 'arb-extra', email: 'extra@example.test', displayName: 'Appeal Member', role: 'requester' as const };
    for (const profile of [low, high, extra]) store.profiles.set(profile.id, profile);
    await store.setArbitrationMember(admin.id, true, admin.id, currentNow, 2);
    await store.setArbitrationMember(low.id, true, admin.id, currentNow, 1);
    await store.setArbitrationMember(high.id, true, admin.id, currentNow, 4);
    ...
    await api(`/api/missions/${mission.id}/workflow`, {
      method: 'POST',
      body: JSON.stringify({ assignments: Object.fromEntries(matches.map((match) => [match.stageId, match.candidates[0].agent.id])) }),
    }, requester.id);
    await acceptAllStageOffers(mission.id);
    await api(`/api/missions/${mission.id}/start`, { method: 'POST', body: '{}' }, requester.id);
    const opened = await api(`/api/missions/${mission.id}/disputes`, {
      method: 'POST',
      body: JSON.stringify({ reason: 'Power 仲裁需要冻结票权并验证只有一轮扩大委员上诉。', evidence: [] }),
    }, requester.id);
    const disputeId = opened.body.data.id as string;

    const review = await api(`/api/disputes/${disputeId}/review`, {
      method: 'POST', body: JSON.stringify({ weightMode: 'power' }),
    }, admin.id);
    expect(review.response.status).toBe(200);
    const initial = await api(`/api/disputes/${disputeId}/governance`, {}, high.id);
    expect(initial.body.data.proposal).toMatchObject({ round: 0, weightMode: 'power', weightVersion: 'member_power.v1', eligibleWeight: 7, quorumRequired: 5 });
    expect(initial.body.data.electorate.find((item: JsonBody) => item.userId === high.id)).toMatchObject({ powerSnapshot: 4, voteWeight: 4 });

    await store.setArbitrationMember(high.id, true, admin.id, currentNow, 9);
    expect((await api(`/api/disputes/${disputeId}/governance`, {}, high.id)).body.data.electorate.find(
      (item: JsonBody) => item.userId === high.id,
    )).toMatchObject({ powerSnapshot: 4, voteWeight: 4 });
    await api(`/api/disputes/${disputeId}/votes`, {
      method: 'POST', body: JSON.stringify({ choice: 'support_refund', reason: '高 Power 委员依据交付证据支持退款，但尚未达到冻结法定票权。' }),
    }, high.id);
    const finalInitial = await api(`/api/disputes/${disputeId}/votes`, {
      method: 'POST', body: JSON.stringify({ choice: 'support_refund', reason: '管理员委员复核证据后支持退款，使首轮形成不可逆多数。' }),
    }, admin.id);
    expect(finalInitial.body.data.proposal.status).toBe('succeeded');

    const noExpansion = await api(`/api/disputes/${disputeId}/appeal`, {
      method: 'POST', body: JSON.stringify({ reason: '首轮证据解释存在分歧，但当前委员会尚未满足扩大快照要求。' }),
    }, requester.id);
    expect(noExpansion.response.status).toBe(409);
    expect(noExpansion.body.error.code).toBe('ARBITRATION_APPEAL_NEEDS_EXPANDED_COUNCIL');
    expect((await store.createDisputeAppeal(
      disputeId, requester.id, '超过上诉期限的请求必须失败且不能创建任何新的提案轮次。', '2026-08-20T00:00:00.000Z',
    )).state).toBe('expired');

    await store.setArbitrationMember(extra.id, true, admin.id, currentNow, 3);
    const sourceDispute = store.disputes.get(mission.id)!.find((item) => item.id === disputeId)!;
    const sourceProposal = store.disputeProposals.get(disputeId)![0];
    const sourceElectorate = store.disputeElectorate.get(sourceProposal.id)!;
    const nonDirectionalId = 'DSP-MEMORY-NON-DIRECTIONAL';
    const nonDirectionalProposalId = 'PROP-MEMORY-NON-DIRECTIONAL';
    store.disputes.set(mission.id, [...store.disputes.get(mission.id)!, { ...sourceDispute, id: nonDirectionalId }]);
    store.disputeProposals.set(nonDirectionalId, [{
      ...sourceProposal,
      id: nonDirectionalProposalId,
      disputeId: nonDirectionalId,
      status: 'quorum_failed',
      outcome: null,
    }]);
    store.disputeElectorate.set(nonDirectionalProposalId, sourceElectorate);
    store.disputeVotes.set(nonDirectionalProposalId, []);
    expect((await store.getDisputeGovernance(nonDirectionalId, requester.id, currentNow))?.appeal.canAppeal).toBe(true);
    expect((await store.createDisputeAppeal(
      nonDirectionalId, requester.id, '首轮未达到法定票权时仍应允许当事人在窗口内请求扩大委员会复核。', currentNow,
    )).state).toBe('created');

    const queuedRaceId = 'DSP-MEMORY-QUEUED-RACE';
    const queuedRaceProposalId = 'PROP-MEMORY-QUEUED-RACE';
    store.disputes.set(mission.id, [...store.disputes.get(mission.id)!, { ...sourceDispute, id: queuedRaceId }]);
    store.disputeProposals.set(queuedRaceId, [{ ...sourceProposal, id: queuedRaceProposalId, disputeId: queuedRaceId }]);
    store.disputeElectorate.set(queuedRaceProposalId, sourceElectorate);
    store.disputeVotes.set(queuedRaceProposalId, []);
    store.governanceExecutionQueue.set(queuedRaceId, {
      id: 'GEXEC-MEMORY-QUEUED-RACE', scope: 'task_dispute', sourceId: queuedRaceId,
      proposalId: queuedRaceProposalId, action: 'refund_requester', payloadHash: 'sha256:test', status: 'queued',
      requestedBy: admin.id, requestedAt: currentNow, txHash: null, executedBy: null, executedAt: null,
    });
    expect((await store.getDisputeGovernance(queuedRaceId, requester.id, currentNow))?.appeal.canAppeal).toBe(false);
    expect((await store.createDisputeAppeal(
      queuedRaceId, requester.id, '即使请求时间仍在窗口内，已经入队的首轮结果也必须与上诉创建互斥。', currentNow,
    )).state).toBe('execution_queued');

    const concurrentRaceId = 'DSP-MEMORY-CONCURRENT-RACE';
    const concurrentRaceProposalId = 'PROP-MEMORY-CONCURRENT-RACE';
    store.disputes.set(mission.id, [...store.disputes.get(mission.id)!, { ...sourceDispute, id: concurrentRaceId }]);
    store.disputeProposals.set(concurrentRaceId, [{ ...sourceProposal, id: concurrentRaceProposalId, disputeId: concurrentRaceId }]);
    store.disputeElectorate.set(concurrentRaceProposalId, sourceElectorate);
    store.disputeVotes.set(concurrentRaceProposalId, []);
    const [concurrentQueue, concurrentAppeal] = await Promise.all([
      store.queueDisputeExecution(concurrentRaceId, admin.id, '2026-08-25T00:00:00.000Z', false),
      store.createDisputeAppeal(
        concurrentRaceId, requester.id, '窗口内上诉与过期后入队并发时只能有一个状态转换成功。', currentNow,
      ),
    ]);
    expect(concurrentAppeal.state).toBe('created');
    expect(concurrentQueue.state).toBe('not_ready');
    expect(store.governanceExecutionQueue.has(concurrentRaceId)).toBe(false);
    expect(store.disputeProposals.get(concurrentRaceId)?.at(-1)).toMatchObject({ round: 1, status: 'active' });

    const outsiderAppeal = await api(`/api/disputes/${disputeId}/appeal`, {
      method: 'POST', body: JSON.stringify({ reason: '仲裁委员不是案件当事方，不能代替任务参与方发起上诉申请。' }),
    }, extra.id);
    expect(outsiderAppeal.response.status).toBe(403);
    expect(outsiderAppeal.body.error.code).toBe('ARBITRATION_APPEAL_NOT_ALLOWED');
    const appealed = await api(`/api/disputes/${disputeId}/appeal`, {
      method: 'POST', body: JSON.stringify({ reason: '首轮对交付验收证据的解释存在重大分歧，请扩大无利益冲突委员会重新复核。' }),
    }, requester.id);
    expect(appealed.response.status).toBe(201);
    expect(appealed.body.data.proposal).toMatchObject({ round: 1, parentProposalId: finalInitial.body.data.proposal.id, eligibleWeight: 15, quorumRequired: 9 });
    expect(appealed.body.data.rounds).toHaveLength(2);
    expect(appealed.body.data.rounds[0].electorate.find((item: JsonBody) => item.userId === high.id)).toMatchObject({ voteWeight: 4 });
    expect(appealed.body.data.electorate.find((item: JsonBody) => item.userId === high.id)).toMatchObject({ voteWeight: 9 });
    expect(appealed.body.data.electorate.map((item: JsonBody) => item.userId)).toContain(extra.id);

    const repeatedAppeal = await api(`/api/disputes/${disputeId}/appeal`, {
      method: 'POST', body: JSON.stringify({ reason: '同一案件不能创建第二轮上诉，也不能覆盖已经冻结的两轮投票记录。' }),
    }, developer.id);
    expect(repeatedAppeal.response.status).toBe(409);
    expect(repeatedAppeal.body.error.code).toBe('ARBITRATION_APPEAL_USED');
    const blockedQueue = await api(`/api/disputes/${disputeId}/execution`, { method: 'POST', body: '{}' }, admin.id);
    expect(blockedQueue.response.status).toBe(409);

    const appealVote = await api(`/api/disputes/${disputeId}/votes`, {
      method: 'POST', body: JSON.stringify({ choice: 'oppose_refund', reason: '扩大委员复核后认为交付证据足以驳回退款请求并恢复任务。' }),
    }, high.id);
    expect(appealVote.body.data.proposal).toMatchObject({ round: 1, status: 'defeated', outcome: 'reject_dispute' });
    expect(appealVote.body.data.executionReady).toBe(true);
    const frozenEscrow = store.escrows.get(mission.id)!;
    store.escrows.set(mission.id, { ...frozenEscrow, status: 'held' });
    const unfrozenQueue = await api(`/api/disputes/${disputeId}/execution`, { method: 'POST', body: '{}' }, admin.id);
    expect(unfrozenQueue.response.status).toBe(409);
    expect(unfrozenQueue.body.error.code).toBe('ESCROW_NOT_FROZEN');
    store.escrows.set(mission.id, frozenEscrow);
    const queued = await api(`/api/disputes/${disputeId}/execution`, { method: 'POST', body: '{}' }, admin.id);
    expect(queued.response.status).toBe(201);
    expect(queued.body.data.execution).toMatchObject({ action: 'reject_dispute', status: 'queued' });
    const resolved = await api(`/api/disputes/${disputeId}/resolve`, {
      method: 'POST', body: JSON.stringify({ status: 'rejected', resolution: '上诉轮最终驳回争议，按队列授权恢复被冻结的任务托管。' }),
    }, admin.id);
    expect(resolved.response.status).toBe(200);
    expect(resolved.body.data.status).toBe('rejected');
    expect((await store.getEscrow(mission.id))?.status).toBe('held');
  });
  ...
  it('serializes concurrent dispute creation and accepts only the first ruling', async () => {
    ...
    expect((await store.listDisputeActions(disputeId))).toHaveLength(4);
  ...
  it('routes large ledger exports through a private async job state machine', async () => {
    for (let index = 0; index < 5_001; index += 1) {
      store.ledgerEntries.push({
        id: `large-ledger-${index}`, missionId: `large-mission-${index}`, missionTitle: `Private row ${index}`,
        agentId: 'research-agent', agentName: 'Research Agent', entryType: 'agent_payout', amount: index + 1,
        token: 'mUSDC', status: 'settled', txHash: null, createdAt: '2026-08-16T00:00:00.000Z',
      });
    }

    const direct = await api('/api/developer/ledger/export-jobs', {
      method: 'POST', headers: { 'Idempotency-Key': 'small-ledger-export-0001' }, body: JSON.stringify({ token: 'sETH' }),
    }, developer.id);
    expect(direct.response.status).toBe(200);
    expect(direct.body.data).toEqual({ mode: 'direct', rowCount: 0 });

    const request = {
      method: 'POST', headers: { 'Idempotency-Key': 'large-ledger-export-0001' }, body: JSON.stringify({ token: 'mUSDC' }),
    };
    const created = await api('/api/developer/ledger/export-jobs', request, developer.id);
    const replay = await api('/api/developer/ledger/export-jobs', request, developer.id);
    expect(created.response.status).toBe(202);
    expect(created.body.data).toMatchObject({ mode: 'async', job: { status: 'queued', totalRows: 5_001, progress: 0, attempt: 1 } });
    expect(replay.body.data.job.id).toBe(created.body.data.job.id);
    expect(replay.body.meta.replayed).toBe(true);
    const exportId = created.body.data.job.id as string;
    expect((await api(`/api/developer/ledger/export-jobs/${exportId}`, {}, otherDeveloper.id)).response.status).toBe(404);

    const disabled = await api('/api/internal/export-jobs', {
      method: 'POST', body: JSON.stringify({ workerId: 'export-worker-1' }),
    });
    expect(disabled.response.status).toBe(503);
    expect(disabled.body.error.code).toBe('EXPORT_SERVICE_NOT_CONFIGURED');

    testEnv.EXPORT_SERVICE_TOKEN = 'private-export-service-test-token';
    const serviceHeaders = { 'X-Export-Service-Token': testEnv.EXPORT_SERVICE_TOKEN };
    const claimed = await api('/api/internal/export-jobs', {
      method: 'POST', headers: serviceHeaders, body: JSON.stringify({ workerId: 'export-worker-1' }),
    });
    expect(claimed.body.data).toMatchObject({ ownerId: developer.id, job: { id: exportId, status: 'processing' } });
    const firstAttempt = claimed.body.data.job.attempt as number;
    expect(JSON.stringify(claimed.body)).not.toContain('Private row');
    expect((await api(`/api/internal/export-jobs/${exportId}/progress`, {
      method: 'POST', headers: serviceHeaders, body: JSON.stringify({ workerId: 'export-worker-1', attempt: firstAttempt, processedRows: 2_500 }),
    })).body.data).toMatchObject({ processedRows: 2_500, progress: 49 });
    expect((await api(`/api/internal/export-jobs/${exportId}/fail`, {
      method: 'POST', headers: serviceHeaders, body: JSON.stringify({ workerId: 'export-worker-1', attempt: firstAttempt, errorCode: 'STORAGE_ERROR', errorMessage: 'must not be persisted' }),
    })).body.data).toMatchObject({ status: 'failed', errorCode: 'STORAGE_ERROR', errorMessage: 'Private export storage failed' });

    const retried = await api(`/api/developer/ledger/export-jobs/${exportId}/retry`, {
      method: 'POST', headers: { 'Idempotency-Key': 'large-ledger-retry-0001' }, body: '{}',
    }, developer.id);
    expect(retried.body.data).toMatchObject({ status: 'queued', attempt: 2, processedRows: 0, progress: 0 });
    const secondClaim = await api('/api/internal/export-jobs', {
      method: 'POST', headers: serviceHeaders, body: JSON.stringify({ workerId: 'export-worker-2' }),
    });
    const secondAttempt = secondClaim.body.data.job.attempt as number;
    const completed = await api(`/api/internal/export-jobs/${exportId}/complete`, {
      method: 'POST', headers: serviceHeaders, body: JSON.stringify({
        workerId: 'export-worker-2', attempt: secondAttempt, objectKey: `private/exports/${exportId}/ledger.csv`, sha256: 'a'.repeat(64),
        rowCount: 5_001, byteSize: 123_456,
      }),
    });
    expect(completed.body.data).toMatchObject({ status: 'completed', progress: 100, artifact: { rowCount: 5_001, byteSize: 123_456 } });
    expect(JSON.stringify(completed.body)).not.toContain('private/exports');
    const downloadDisabled = await api(`/api/developer/ledger/export-jobs/${exportId}/download-token`, {
      method: 'POST', body: '{}',
    }, developer.id);
    expect(downloadDisabled.response.status).toBe(503);
    expect(downloadDisabled.body.error.code).toBe('EXPORT_STORAGE_NOT_CONFIGURED');

    let signedObjectKey: string | null = null;
    const signingApp = createApp({
      storeFactory: () => store,
      identityResolver: async (request) => {
        const uid = request.headers.get('X-Test-User');
        const profile = uid ? store.profiles.get(uid) : null;
        return uid ? { uid, provider: 'pinme', email: profile?.email, displayName: profile?.displayName, claims: {} } : null;
      },
      now: () => new Date(currentNow),
      exportDownloadIssuer: async (_env, artifact, expiresAt) => {
        signedObjectKey = artifact.objectKey;
        return { url: 'https://private-download.example.test/short-lived-token', expiresAt };
      },
    });
    const signedResponse = await signingApp.fetch(new Request(
      `http://local.test/api/developer/ledger/export-jobs/${exportId}/download-token`,
      { method: 'POST', headers: { 'X-Test-User': developer.id, 'Content-Type': 'application/json' }, body: '{}' },
    ), testEnv);
    const signedBody = await signedResponse.json() as JsonBody;
    expect(signedResponse.status).toBe(200);
    expect(signedBody.data.url).toBe('https://private-download.example.test/short-lived-token');
    expect(Date.parse(signedBody.data.expiresAt) - Date.parse(currentNow)).toBe(5 * 60 * 1_000);
    expect(signedObjectKey).toBe(`private/exports/${exportId}/ledger.csv`);

    const invalidSigningApp = createApp({
      storeFactory: () => store,
      identityResolver: async (request) => {
        const uid = request.headers.get('X-Test-User');
        const profile = uid ? store.profiles.get(uid) : null;
      ...
      now: () => new Date(currentNow),
      exportDownloadIssuer: async () => ({
        url: 'https://private-download.example.test/unbounded-token', expiresAt: 'not-a-date',
      }),
    });
    const invalidSignedResponse = await invalidSigningApp.fetch(new Request(
      `http://local.test/api/developer/ledger/export-jobs/${exportId}/download-token`,
      { method: 'POST', headers: { 'X-Test-User': developer.id, 'Content-Type': 'application/json' }, body: '{}' },
    ), testEnv);
    expect(invalidSignedResponse.status).toBe(502);
    expect(((await invalidSignedResponse.json()) as JsonBody).error.code).toBe('INVALID_EXPORT_DOWNLOAD_TOKEN');

    const malformedUrlSigningApp = createApp({
      storeFactory: () => store,
      identityResolver: async (request) => {
        const uid = request.headers.get('X-Test-User');
        const profile = uid ? store.profiles.get(uid) : null;
      ...
      now: () => new Date(currentNow),
      exportDownloadIssuer: async (_env, _artifact, expiresAt) => ({ url: 'https://', expiresAt }),
    });
    const malformedUrlResponse = await malformedUrlSigningApp.fetch(new Request(
      `http://local.test/api/developer/ledger/export-jobs/${exportId}/download-token`,
      { method: 'POST', headers: { 'X-Test-User': developer.id, 'Content-Type': 'application/json' }, body: '{}' },
    ), testEnv);
    expect(malformedUrlResponse.status).toBe(502);
    expect(((await malformedUrlResponse.json()) as JsonBody).error.code).toBe('INVALID_EXPORT_DOWNLOAD_TOKEN');

    let signingClock = currentNow;
    const expiredDuringSigningApp = createApp({
      storeFactory: () => store,
      identityResolver: async (request) => {
        const uid = request.headers.get('X-Test-User');
        const profile = uid ? store.profiles.get(uid) : null;
      ...
      now: () => new Date(signingClock),
      exportDownloadIssuer: async (_env, _artifact, expiresAt) => {
        signingClock = expiresAt;
        return { url: 'https://private-download.example.test/expired-during-signing', expiresAt };
      },
    });
    const expiredDuringSigningResponse = await expiredDuringSigningApp.fetch(new Request(
      `http://local.test/api/developer/ledger/export-jobs/${exportId}/download-token`,
      { method: 'POST', headers: { 'X-Test-User': developer.id, 'Content-Type': 'application/json' }, body: '{}' },
    ), testEnv);
    expect(expiredDuringSigningResponse.status).toBe(502);
    expect(((await expiredDuringSigningResponse.json()) as JsonBody).error.code).toBe('INVALID_EXPORT_DOWNLOAD_TOKEN');

    currentNow = '2026-08-18T00:00:01.000Z';
    const expired = await api(`/api/developer/ledger/export-jobs/${exportId}`, {}, developer.id);
    expect(expired.body.data).toMatchObject({ status: 'expired', artifact: null, expiresAt: '2026-08-17T00:00:00.000Z' });
    const replacement = await api('/api/developer/ledger/export-jobs', {
      method: 'POST', headers: { 'Idempotency-Key': 'large-ledger-export-0002' }, body: JSON.stringify({ token: 'mUSDC' }),
    }, developer.id);
    const replacementId = replacement.body.data.job.id as string;
    const cancelled = await api(`/api/developer/ledger/export-jobs/${replacementId}/cancel`, {
      method: 'POST', headers: { 'Idempotency-Key': 'large-ledger-cancel-0001' }, body: '{}',
    }, developer.id);
    expect(cancelled.body.data.status).toBe('cancelled');
    expect(JSON.stringify(store.ledgerExportEvents)).not.toContain('Private row');
    expect(JSON.stringify(store.ledgerExportEvents)).not.toContain('large-ledger-');
  });

  it('fences stale export attempts and keeps retry and expiry invariants aligned in memory', async () => {
    for (let index = 0; index < 5_001; index += 1) {
      store.ledgerEntries.push({
        id: `fenced-ledger-${index}`, missionId: `fenced-mission-${index}`, missionTitle: `Fenced row ${index}`,
        agentId: 'research-agent', agentName: 'Research Agent', entryType: 'agent_payout', amount: index + 1,
        token: 'mUSDC', status: 'settled', txHash: null, createdAt: '2026-08-16T00:00:00.000Z',
      });
    }

    const created = await store.requestDeveloperLedgerExport(developer.id, 'mUSDC', '2026-08-16T00:00:00.000Z');
    expect(created.mode).toBe('async');
    if (created.mode !== 'async') return;
    const exportId = created.job.id;
    const originalExpiry = created.job.expiresAt;
    const firstClaim = await store.claimDeveloperLedgerExport('reused-worker', '2026-08-16T00:00:00.000Z');
    expect(firstClaim?.job.attempt).toBe(1);
    expect(await store.updateDeveloperLedgerExportProgress(
      exportId, 'reused-worker', 1, 100, '2026-08-16T00:05:00.000Z',
    )).toBeNull();

    const recovered = await store.claimDeveloperLedgerExport('reused-worker', '2026-08-16T00:05:01.000Z');
    expect(recovered?.job).toMatchObject({ id: exportId, status: 'processing', attempt: 2 });
    expect(await store.updateDeveloperLedgerExportProgress(
      exportId, 'reused-worker', 1, 100, '2026-08-16T00:05:02.000Z',
    )).toBeNull();
    expect(await store.completeDeveloperLedgerExport({
      id: exportId, workerId: 'reused-worker', attempt: 1,
      objectKey: `private/exports/${exportId}/stale.csv`, sha256: 'c'.repeat(64),
      rowCount: 5_001, byteSize: 10_000, completedAt: '2026-08-16T00:05:02.000Z',
    })).toBeNull();
    expect(await store.failDeveloperLedgerExport(
      exportId, 'reused-worker', 1, 'STORAGE_ERROR', 'Private export storage failed', '2026-08-16T00:05:02.000Z',
    )).toBeNull();
    expect(await store.updateDeveloperLedgerExportProgress(
      exportId, 'reused-worker', 2, 250, '2026-08-16T00:05:02.000Z',
    )).toMatchObject({ processedRows: 250, attempt: 2 });
    expect(await store.failDeveloperLedgerExport(
      exportId, 'reused-worker', 2, 'STORAGE_ERROR', 'Private export storage failed', '2026-08-16T00:06:00.000Z',
    )).toMatchObject({ status: 'failed', attempt: 2 });

    const sibling = await store.requestDeveloperLedgerExport(developer.id, 'mUSDC', '2026-08-16T00:06:01.000Z');
    expect(sibling.mode).toBe('async');
    if (sibling.mode !== 'async') return;
    expect((await store.retryDeveloperLedgerExport(
      developer.id, exportId, '2026-08-16T00:06:02.000Z',
    )).state).toBe('invalid_state');
    await store.cancelDeveloperLedgerExport(developer.id, sibling.job.id, '2026-08-16T00:06:03.000Z');

    const retried = await store.retryDeveloperLedgerExport(developer.id, exportId, '2026-08-22T00:00:00.000Z');
    expect(retried).toMatchObject({ state: 'applied', job: { status: 'queued', attempt: 3, expiresAt: originalExpiry } });
    const finalClaim = await store.claimDeveloperLedgerExport('expiry-worker', '2026-08-22T00:01:00.000Z');
    expect(finalClaim?.job).toMatchObject({ id: exportId, attempt: 3 });
    const internalJob = store.ledgerExportJobs.get(exportId);
    if (internalJob) internalJob.leaseExpiresAt = '2026-08-24T00:00:00.000Z';
    expect(await store.completeDeveloperLedgerExport({
      id: exportId, workerId: 'expiry-worker', attempt: 3,
      objectKey: `private/exports/${exportId}/late.csv`, sha256: 'd'.repeat(64),
      rowCount: 5_001, byteSize: 10_000, completedAt: '2026-08-23T00:00:01.000Z',
    })).toBeNull();
    expect(await store.getDeveloperLedgerExport(
      developer.id, exportId, '2026-08-23T00:00:01.000Z',
  ...
  it('versions private templates and expands bounded loops before funding', async () => {
    const { mission, stages } = await createMission();
    const saveBody = JSON.stringify({
      missionId: mission.id,
      name: '研究交付链',
      description: '可复用的研究、分析和交付流程。',
    });
    const saveHeaders = { 'Idempotency-Key': 'template-save-replay-0001' };
    const saved = await api('/api/workflow-templates', {
      method: 'POST',
      headers: saveHeaders,
      body: saveBody,
    }, requester.id);
    ...
    expect(saved.body.data).toMatchObject({
      template: { name: '研究交付链', ownerId: requester.id, currentVersion: 1 },
      version: { version: 1, entryIds: ['NODE-1'], exitIds: ['NODE-3'] },
    });
    expect(saved.body.data.version.nodes).toHaveLength(3);
    expect(saved.body.data.version.nodes.every((node: JsonBody) => node.agentId === null && node.status === 'queued')).toBe(true);

    const replayedSave = await api('/api/workflow-templates', {
      method: 'POST', headers: saveHeaders, body: saveBody,
    }, requester.id);
    expect(replayedSave.response.status).toBe(201);
    expect(replayedSave.body).toMatchObject({ data: { version: { version: 1 } }, meta: { replayed: true, unchanged: false } });
    expect((await store.listEvents(mission.id)).filter((event) => event.type === 'workflow.template_saved')).toHaveLength(1);

    const unchanged = await api('/api/workflow-templates', {
      method: 'POST',
      body: JSON.stringify({
        missionId: mission.id,
        name: '研究交付链',
    ...
    expect(unchanged.body).toMatchObject({ data: { version: { version: 1 } }, meta: { unchanged: true } });

    const metadataRevision = await api('/api/workflow-templates', {
      method: 'POST',
      body: JSON.stringify({
        missionId: mission.id,
        name: '研究交付链',
        description: '同一流程内容，但补充了模板说明。',
      }),
    }, requester.id);
    expect(metadataRevision.response.status).toBe(201);
    expect(metadataRevision.body.data).toMatchObject({
      template: { currentVersion: 2, description: '同一流程内容，但补充了模板说明。' },
      version: { version: 2, contentHash: saved.body.data.version.contentHash },
    });

    const ownList = await api('/api/workflow-templates', {}, requester.id);
    const privateList = await api('/api/workflow-templates', {}, otherDeveloper.id);
    expect(ownList.body.data).toHaveLength(1);
    expect(privateList.body.data).toEqual([]);
    ...
    const expandHeaders = { 'Idempotency-Key': 'template-expand-replay-0001' };
    const expanded = await api(`/api/missions/${mission.id}/workflow/expand`, {
      method: 'POST',
      headers: expandHeaders,
      body: expandBody,
    }, requester.id);
    expect(expanded.response.status).toBe(200);
    expect(expanded.body.data.mission.workflowVersion).toBe(2);
    expect(expanded.body.data.stages).toHaveLength(6);
    expect(expanded.body.data.edges).toHaveLength(5);
    expect(expanded.body.data.stages.filter((node: JsonBody) => node.nodeType === 'task')
      .reduce((sum: number, node: JsonBody) => sum + node.budget, 0)).toBe(mission.budget);
    expect(new Set(expanded.body.data.stages.map((node: JsonBody) => node.input.template.iteration))).toEqual(new Set([1, 2]));
    expect(expanded.body.data.stages.every((node: JsonBody) => node.input.template.contentHash === saved.body.data.version.contentHash)).toBe(true);

    const replayedExpansion = await api(`/api/missions/${mission.id}/workflow/expand`, {
      method: 'POST', headers: expandHeaders, body: expandBody,
    }, requester.id);
    expect(replayedExpansion.response.status).toBe(200);
    expect(replayedExpansion.body).toMatchObject({ data: { mission: { workflowVersion: 2 } }, meta: { replayed: true } });
    expect(replayedExpansion.body.data.stages.map((node: JsonBody) => node.id))
      .toEqual(expanded.body.data.stages.map((node: JsonBody) => node.id));
    expect((await store.listEvents(mission.id)).filter((event) => event.type === 'workflow.template_expanded')).toHaveLength(1);

    const stale = await api(`/api/missions/${mission.id}/workflow/expand`, {
      method: 'POST',
      body: JSON.stringify({
        templateId: saved.body.data.template.id,
        iterations: 1,
    ...
    const attached = await api(`/api/missions/${second.mission.id}/workflow/expand`, {
      method: 'POST',
      body: JSON.stringify({
        templateId: saved.body.data.template.id,
        iterations: 1,
        budget: 100,
        workflowVersion: 1,
        attachAfterStageId: second.stages[0].id,
        attachBeforeStageId: second.stages[1].id,
      }),
    }, requester.id);
    expect(attached.response.status, JSON.stringify(attached.body)).toBe(200);
    expect(attached.body.data.stages).toHaveLength(6);
    const attachedTemplateNodes = attached.body.data.stages.filter((node: JsonBody) => node.input.template);
    expect(attachedTemplateNodes.reduce((sum: number, node: JsonBody) => sum + node.budget, 0)).toBe(100);
    expect(attached.body.data.stages.filter((node: JsonBody) => !node.input.template && node.nodeType === 'task')
      .reduce((sum: number, node: JsonBody) => sum + node.budget, 0)).toBe(200);

    const ruled = await createMission();
    const ruledEdges = await store.listEdges(ruled.mission.id);
    const ruledDraft = await api(`/api/missions/${ruled.mission.id}/workflow/draft`, {
      method: 'PUT',
      body: JSON.stringify({
        workflowVersion: 1,
        nodes: ruled.stages,
        edges: ruledEdges.map((edge, index) => ({
          id: edge.id,
          sourceStageId: edge.sourceStageId,
          targetStageId: edge.targetStageId,
          mappings: index === 0 ? [{ from: '/summary', to: '/summary', required: true }] : [],
        })),
        viewport: { x: 0, y: 0, zoom: 1 },
      }),
    }, requester.id);
    expect(ruledDraft.response.status).toBe(200);
    const rejectedAttachment = await api(`/api/missions/${ruled.mission.id}/workflow/expand`, {
      method: 'POST',
      body: JSON.stringify({
        templateId: saved.body.data.template.id,
        iterations: 1,
        budget: 100,
        workflowVersion: 2,
        attachAfterStageId: ruledEdges[0].sourceStageId,
        attachBeforeStageId: ruledEdges[0].targetStageId,
      }),
    }, requester.id);
    expect(rejectedAttachment.response.status).toBe(400);
    expect(rejectedAttachment.body.error.code).toBe('TEMPLATE_ATTACHMENT_RULED_EDGE');
    expect((await store.getMission(ruled.mission.id))?.workflowVersion).toBe(2);
  });

  it('promotes historical private-template content as a new current memory revision', async () => {
    const { mission, stages } = await createMission();
    const edges = await store.listEdges(mission.id);
    const base = {
      id: 'TEMPLATE-memory-history', ownerId: requester.id, name: 'Memory revision history', description: 'First revision',
      nodes: stages, edges, entryIds: [stages[0].id], exitIds: [stages.at(-1)!.id],
      contentHash: `sha256:${'a'.repeat(64)}`, createdAt: '2026-08-16T00:01:00.000Z',
    };
    expect(await store.saveWorkflowTemplateVersion(base)).toMatchObject({ state: 'saved', detail: { version: { version: 1 } } });
    expect(await store.saveWorkflowTemplateVersion({
      ...base, description: 'Different content', contentHash: `sha256:${'b'.repeat(64)}`, createdAt: '2026-08-16T00:02:00.000Z',
    })).toMatchObject({ state: 'saved', detail: { version: { version: 2 } } });
    expect(await store.saveWorkflowTemplateVersion({
      ...base, description: 'Historical content restored', createdAt: '2026-08-16T00:03:00.000Z',
    })).toMatchObject({
      state: 'saved',
      detail: { template: { currentVersion: 3, description: 'Historical content restored' }, version: { version: 3, contentHash: base.contentHash } },
    });
    expect(await store.getWorkflowTemplate(requester.id, base.id, 1)).toMatchObject({ version: { version: 1 } });
    expect(await store.getWorkflowTemplate(requester.id, base.id)).toMatchObject({ version: { version: 3 } });
  });

  it('bounds mission detail to transition checkpoints from the current source attempt', async () => {
    const { mission } = await createMission();
    const edge = (await store.listEdges(mission.id))[0];
    const transitions: WorkflowTransitionCheckpoint[] = Array.from({ length: 205 }, (_, index) => ({
      id: `TRANSITION-detail-${index + 1}`,
      missionId: mission.id,
      edgeId: edge.id,
      sourceStageId: edge.sourceStageId,
      targetStageId: edge.targetStageId,
      sourceAttemptNo: index + 1,
      workflowVersion: mission.workflowVersion,
      matched: true,
      mappedInput: { sequence: index + 1 },
      missingRequired: [],
      errorCode: null,
      createdAt: new Date(Date.parse('2026-08-16T01:00:00.000Z') + index * 1_000).toISOString(),
    }));
    store.workflowTransitions.set(mission.id, transitions);

    const detail = await api(`/api/missions/${mission.id}`, {}, requester.id);
    expect(detail.response.status).toBe(200);
    expect(detail.body.data.transitions).toHaveLength(1);
    expect(detail.body.data.transitions[0]).toMatchObject({ sourceAttemptNo: 1, mappedInput: { sequence: 1 } });
    expect(await store.listWorkflowTransitions(mission.id)).toHaveLength(205);
  });
  ...
  it('fans out root nodes, joins direct handoffs, and pauses at an approval Gate', async () => {
    ...
    const edges = [
      {
        ...
        mappings: [
          { from: '/summary', to: '/research/aSummary', required: true },
          { from: '/customer/id', to: '/research/customerId', required: true },
        ],
      },
      {
        id: 'EDGE-b-c', sourceStageId: stages[1].id, targetStageId: stages[2].id,
        mappings: [{ from: '/findings/0', to: '/research/bEvidence', required: true }],
      },
      {
        id: 'EDGE-c-g', sourceStageId: stages[2].id, targetStageId: gate.id,
        condition: { op: 'eq', path: '/customer/id', value: `customer-${stages[2].id}` },
      },
    ];
    const saved = await api(`/api/missions/${mission.id}/workflow/draft`, {
      method: 'PUT', body: JSON.stringify({ workflowVersion: 1, nodes, edges, viewport: { x: 0, y: 0, zoom: 1 } }),
      ...
      body: JSON.stringify({
        ...
        output: {
          ...
          result: {
            ...
            findings: ['可验证结果'],
            customer: { id: `customer-${body.task.node.id}`, tier: 'enterprise' },
    ...
    expect(dispatchedBodies[0].task.mappedInput).toEqual({
      research: {
        aSummary: `Output for ${stages[0].id}`,
        bEvidence: '可验证结果',
        customerId: `customer-${stages[0].id}`,
      },
    });
    expect(dispatchedBodies[0].task.transitionCheckpoints).toHaveLength(2);
    ...
    expect(waiting.body.data.transitions).toEqual(expect.arrayContaining([
      expect.objectContaining({
        edgeId: 'EDGE-a-c', matched: true,
        mappedInput: { research: { aSummary: `Output for ${stages[0].id}`, customerId: `customer-${stages[0].id}` } },
      }),
      expect.objectContaining({ edgeId: 'EDGE-b-c', matched: true, mappedInput: { research: { bEvidence: '可验证结果' } } }),
      expect.objectContaining({ edgeId: 'EDGE-c-g', matched: true }),
    ]));
    ...
    expect((await api(`/api/missions/${mission.id}/pause`, {
      method: 'POST', body: JSON.stringify({ reason: '审批前发现汇合结果需要返工。' }),
    }, requester.id)).response.status).toBe(200);
    const pausedRework = await api(`/api/missions/${mission.id}/change-requests`, {
      method: 'POST',
      body: JSON.stringify({
        targetStageIds: [stages[2].id],
        reason: '汇合结果缺少最新的依赖风险证据。',
        acceptanceCriteria: '重新执行汇合节点并再次进入人工审批。',
      }),
    }, requester.id);
    expect(pausedRework.response.status).toBe(201);
    expect(pausedRework.body.data.stages.find((stage: JsonBody) => stage.id === stages[2].id).status).toBe('queued');
    expect(pausedRework.body.data.stages.find((stage: JsonBody) => stage.id === gate.id).status).toBe('queued');
    expect((await api(`/api/missions/${mission.id}/resume`, { method: 'POST', body: '{}' }, requester.id)).response.status).toBe(200);

    dispatchedBodies = [];
    const resumedRework = await api(`/api/missions/${mission.id}/dispatch`, { method: 'POST', body: '{}' }, requester.id);
    expect(resumedRework.response.status).toBe(202);
    expect(resumedRework.body.data.stage.id).toBe(stages[2].id);
    expect((await complete(dispatchedBodies[0], 'callback-dag-paused-rework')).response.status).toBe(202);
  ...
  it('rejects the aggregate mapped input when individually bounded edges exceed the dispatch limit together', async () => {
    const { mission, stages } = await createMission();
    const edges = [
      {
        id: 'EDGE-large-left', sourceStageId: stages[0].id, targetStageId: stages[2].id,
        mappings: [{ from: '/summary', to: '/left', required: true }],
      },
      {
        id: 'EDGE-large-right', sourceStageId: stages[1].id, targetStageId: stages[2].id,
        mappings: [{ from: '/summary', to: '/right', required: true }],
      },
    ];
    const saved = await api(`/api/missions/${mission.id}/workflow/draft`, {
      method: 'PUT', body: JSON.stringify({ workflowVersion: 1, nodes: stages, edges, viewport: { x: 0, y: 0, zoom: 1 } }),
    }, requester.id);
    expect(saved.response.status).toBe(200);
    const candidates = await api(`/api/missions/${mission.id}/candidates`, {}, requester.id);
    const assignments = Object.fromEntries((candidates.body.data as CandidateMatch[])
      .map((match) => [match.stageId, match.candidates[0].agent.id]));
    expect((await api(`/api/missions/${mission.id}/workflow`, {
      method: 'POST', body: JSON.stringify({ assignments }),
    }, requester.id)).response.status).toBe(200);
    await acceptAllStageOffers(mission.id);
    expect((await api(`/api/missions/${mission.id}/start`, { method: 'POST', body: '{}' }, requester.id)).response.status).toBe(200);
    await store.updateStage(mission.id, stages[0].id, 'done', { summary: 'a'.repeat(9_000) });
    await store.updateStage(mission.id, stages[1].id, 'done', { summary: 'b'.repeat(9_000) });

    const dispatched = await api(`/api/missions/${mission.id}/dispatch`, { method: 'POST', body: '{}' }, requester.id);
    expect(dispatched.response.status).toBe(413);
    expect(dispatched.body.error.code).toBe('MAPPED_INPUT_TOO_LARGE');
    expect((await store.listStages(mission.id)).find((stage) => stage.id === stages[2].id)).toMatchObject({
      status: 'queued',
    });
  ...
  it('dispatches the next stage and accepts only its signed Agent callback', async () => {
    ...
    const callback = await api(`/api/hooks/agents/${dispatched.body.data.agent.id}/events`, {
      ...
      body: JSON.stringify({
        ...
        output: {
          summary: 'Callback test output', verified: true,
          customer: { id: 'customer-42', tier: 'enterprise' },
    ...
    expect(callback.body.data.stage.output.customer).toEqual({ id: 'customer-42', tier: 'enterprise' });
  ...
  it('rejects oversized custom Agent callback output before persisting it', async () => {
    const { mission } = await createMission();
    const candidates = await api(`/api/missions/${mission.id}/candidates`, {}, requester.id);
    const matches = candidates.body.data as CandidateMatch[];
    const assignments = Object.fromEntries(matches.map((match) => [match.stageId, match.candidates[0].agent.id]));
    ...
    const callback = await api(`/api/hooks/agents/${dispatched.body.data.agent.id}/events`, {
      ...
      body: JSON.stringify({
        ...
        stageId: dispatched.body.data.stage.id,
        runId: dispatchedBody?.callback.runId,
        callbackId: 'callback-oversized-custom-output',
        expiresAt: dispatchedBody?.callback.expiresAt,
        status: 'done',
        progress: 100,
        output: { summary: 'Callback output', verified: true, customer: { notes: 'x'.repeat(16_001) } },
      }),
    });

    expect(callback.response.status).toBe(400);
    expect(callback.body.error.code).toBe('CALLBACK_OUTPUT_TOO_LARGE');
    expect((await store.listStages(mission.id)).find((stage) => stage.id === dispatched.body.data.stage.id)?.output).toBeNull();
  });
  ...
  it('recovers a stale outbox claim with the same Agent task run ID', async () => {
    ...
    currentNow = '2026-08-16T00:06:00.000Z';
  ...
  it('rotates an expired pending outbox run when a paused mission resumes', async () => {
    const { mission } = await createMission();
    const candidates = await api(`/api/missions/${mission.id}/candidates`, {}, requester.id);
    const matches = candidates.body.data as CandidateMatch[];
    const assignments = Object.fromEntries(matches.map((match) => [match.stageId, match.candidates[0].agent.id]));
    ...
    const original = (await store.listPendingDispatches(10, currentNow))[0];
    expect(original).toBeTruthy();
    expect((await api(`/api/missions/${mission.id}/pause`, {
      method: 'POST', body: JSON.stringify({ reason: '等待外部资料超过原回调有效期。' }),
    }, requester.id)).response.status).toBe(200);

    currentNow = '2026-08-16T03:00:00.000Z';
    expect((await api(`/api/missions/${mission.id}/resume`, { method: 'POST', body: '{}' }, requester.id)).response.status).toBe(200);
    const refreshed = [...store.dispatchOutbox.values()].find((item) => item.id === original.id)!;
    expect(refreshed.runId).not.toBe(original.runId);
    expect(refreshed.expiresAt).toBe('2026-08-16T05:00:00.000Z');

    const dispatched = await api(`/api/missions/${mission.id}/dispatch`, { method: 'POST', body: '{}' }, requester.id);
    expect(dispatched.response.status).toBe(202);
    expect(dispatchedBody?.callback.runId).toBe(refreshed.runId);
    expect(dispatchedBody?.callback.expiresAt).toBe(refreshed.expiresAt);
  });

  it('recovers an expired running attempt with one fresh run when a paused mission resumes', async () => {
    const { mission } = await createMission();
    const candidates = await api(`/api/missions/${mission.id}/candidates`, {}, requester.id);
    const matches = candidates.body.data as CandidateMatch[];
    const assignments = Object.fromEntries(matches.map((match) => [match.stageId, match.candidates[0].agent.id]));
    ...
    const dispatched = await api(`/api/missions/${mission.id}/dispatch`, { method: 'POST', body: '{}' }, requester.id);
    expect(dispatched.response.status).toBe(202);
    const originalRunId = dispatchedBody?.callback.runId as string;
    const originalStageId = dispatched.body.data.stage.id as string;
    expect((await store.listStages(mission.id)).find((stage) => stage.id === originalStageId)).toMatchObject({ status: 'running' });
    expect((await api(`/api/missions/${mission.id}/pause`, {
      method: 'POST', body: JSON.stringify({ reason: '暂停时间超过当前 Agent 回调有效期。' }),
    }, requester.id)).response.status).toBe(200);

    currentNow = '2026-08-16T03:00:00.000Z';
    expect((await api(`/api/missions/${mission.id}/resume`, { method: 'POST', body: '{}' }, requester.id)).response.status).toBe(200);
    const refreshed = [...store.dispatchOutbox.values()].find((item) => item.stageId === originalStageId)!;
    expect(refreshed).toMatchObject({ status: 'pending', expiresAt: '2026-08-16T05:00:00.000Z' });
    expect(refreshed.runId).not.toBe(originalRunId);
  ...
  it('lets an admin mission owner recover an interrupted official Agent run', async () => {
    ...
    store.agents.set('official-strategy-analyst', {
      ...agent('official-strategy-analyst', '商业分析', 12),
      ownerId: officialOwner.id,
      name: 'Strategy Analyst',
      endpoint: 'agentmesh://builtin/analysis',
      official: true,
    });
    const { mission, stages } = await createMission();
    const assignments = Object.fromEntries(stages.map((stage) => [stage.id, 'official-strategy-analyst']));
    expect((await api(`/api/missions/${mission.id}/workflow`, {
      method: 'POST', body: JSON.stringify({ assignments }),
    }, requester.id)).response.status).toBe(200);
    expect((await api(`/api/missions/${mission.id}/start`, {
      method: 'POST', body: '{}',
    }, requester.id)).response.status).toBe(200);
    const target = stages[0];
    expect(await store.claimStageForDispatch(mission.id, target.id)).toMatchObject({ status: 'running' });
    store.profiles.set(requester.id, { ...requester, role: 'admin' });

    const recovered = await api(`/api/missions/${mission.id}/nodes/${target.id}/retry`, {
      method: 'POST', body: '{}',
    }, requester.id);

    expect(recovered.response.status).toBe(200);
    expect(recovered.body.data.stages).toContainEqual(expect.objectContaining({
      id: target.id,
      status: 'queued',
      attemptNo: 2,
    }));
    expect((await store.listEvents(mission.id))).toContainEqual(expect.objectContaining({
      stageId: target.id,
      type: 'node.stuck_recovered',
    }));
  });
  ...
  it('normalizes object-shaped official Agent output into the delivery contract', async () => {
    const official = {
      ...agent('official-evidence-scout', '数据研究', 12),
      ownerId: 'agentmesh-official',
      name: 'Evidence Scout',
    ...
    store.agents.set(official.id, official);
    injectedLlmContent = JSON.stringify({
      summary: { status: '需要补充材料', conclusion: '当前证据不足' },
      findings: { missingSources: ['原始数据', '品牌规范'], reviewResult: '退回复核' },
      recommendation: { nextStep: '补齐来源后重新核验' },
    });

    const invoked = await api('/api/agents/official-evidence-scout/invoke', {
      method: 'POST',
      body: JSON.stringify({ task: '核验交付结果' }),
    }, requester.id);

    expect(invoked.response.status).toBe(200);
    expect(invoked.body.meta.source).toBe('pinme-llm');
    expect(invoked.body.data.result).toMatchObject({
      summary: expect.any(String),
      findings: [expect.any(String)],
      recommendation: expect.any(String),
    });
    expect(invoked.body.data.result.summary).toContain('当前证据不足');
    expect(invoked.body.data.result.findings[0]).toContain('missingSources');
  });

  it('records analysis work as an internal workstream without publishing a fake client artifact', async () => {
    const officialOwner: UserContext = {
      id: 'agentmesh-official', displayName: 'AgentMesh Official', role: 'developer',
    };
    store.profiles.set(officialOwner.id, officialOwner);
    ...
    const { mission, stages } = await createMission();
    expect(stages[0].input.executionMode).toBe('analyze');
    ...
    const dispatched = await api(`/api/missions/${mission.id}/dispatch`, {
      method: 'POST', body: '{}',
    }, requester.id);

    expect(dispatched.response.status).toBe(202);
    expect(dispatched.body.data.acknowledgement).toMatchObject({
      completed: true,
      artifactPublished: false,
      packageKind: 'working_output',
    });
    expect(publishedBundles).toEqual([]);
    expect(await store.listDeliverables(mission.id)).toEqual([]);
    expect((await store.listStages(mission.id))[0]).toMatchObject({ status: 'done' });
  });

  it('queues the first official Agent batch without tying it to the HTTP response lifetime', async () => {
    const officialOwner: UserContext = {
      id: 'agentmesh-official', displayName: 'AgentMesh Official', role: 'developer',
    };
    store.profiles.set(officialOwner.id, officialOwner);
    ...
    const response = await app.fetch(new Request(`http://local.test/api/missions/${mission.id}/start`, {
      method: 'POST', headers: { 'Content-Type': 'application/json', 'X-Test-User': requester.id }, body: '{}',
    }), testEnv, { waitUntil: vi.fn() } as never);

    expect(response.status).toBe(200);
    expect(requestedLlmInputs).toHaveLength(0);
    expect((await store.listStages(mission.id))[0]).toMatchObject({ status: 'queued' });
    expect((await store.listStages(mission.id))[1]).toMatchObject({ status: 'queued' });
    expect(await store.listPendingDispatches(10, currentNow)).toContainEqual(expect.objectContaining({
      missionId: mission.id,
      stageId: stages[0].id,
      status: 'pending',
    }));

    const dispatchResponse = await app.fetch(new Request(`http://local.test/api/missions/${mission.id}/dispatch`, {
      method: 'POST', headers: { 'Content-Type': 'application/json', 'X-Test-User': requester.id }, body: '{}',
    }), testEnv, { waitUntil: vi.fn() } as never);
    expect(dispatchResponse.status).toBe(202);
    expect((await dispatchResponse.json() as JsonBody).data.acknowledgement).toMatchObject({ completed: true });
    expect(requestedLlmInputs).toHaveLength(1);
    expect((await store.listStages(mission.id))[0]).toMatchObject({ status: 'done' });
  });
  ...
  it('auto-accepts and completes stages assigned to runnable official test Agents', async () => {
    ...
    const requesterPinmeAppKey = '0x1234567890abcdef-requester-pinme-token';
    expect((await api('/api/me/integrations/pinme', {
      method: 'PUT', body: JSON.stringify({ appKey: requesterPinmeAppKey }),
    }, requester.id)).response.status).toBe(200);
    const { mission, stages } = await createMission();
    const edges = await store.listEdges(mission.id);
    const draft = await api(`/api/missions/${mission.id}/workflow/draft`, {
      method: 'PUT',
      body: JSON.stringify({
        workflowVersion: 1,
        nodes: stages,
        edges: edges.map((edge, index) => ({
          ...
          sourceStageId: edge.sourceStageId,
          targetStageId: edge.targetStageId,
          mappings: index === 0 ? [{ from: '/summary', to: '/priorSummary', required: true }] : [],
        })),
        viewport: { x: 0, y: 0, zoom: 1 },
      }),
    }, requester.id);
    ...
    expect(started.response.status).toBe(200);
    requestedLlmInputs = [];
    const callbackSpy = vi.spyOn(store, 'applyAgentCallback');
    ...
    for (const [index] of stages.entries()) {
      ...
      if (index === 1) {
        expect(requestedLlmInputs.at(-1)?.mappedInput).toEqual({
          priorSummary: 'Official test Agent completed the assigned stage.',
        });
        expect(requestedLlmInputs.at(-1)?.transitionCheckpoints).toEqual([
          expect.objectContaining({ edgeId: edges[0].id, matched: true, sourceAttemptNo: 1 }),
        ]);
      }
    }

    expect(dispatchCalls).toBe(0);
    expect(callbackSpy).toHaveBeenCalledTimes(stages.length);
    expect(callbackSpy.mock.calls.map(([update]) => update.artifacts?.length)).toEqual([0, 1, 1]);
    expect(publishedBundles).toHaveLength(2);
    expect(publishedPinmeAppKeys).toEqual([requesterPinmeAppKey, requesterPinmeAppKey]);
    expect(publishedBundles[0].paths).toEqual(['deliverable.md', 'index.html', 'manifest.json']);
    expect(publishedBundles[1].paths).toEqual(expect.arrayContaining([
      'deliverable.md', 'acceptance-report.md', 'artifact-index.md', 'index.html', 'manifest.json',
    ]));
    expect(publishedBundles[1].paths.filter((path) => path.startsWith('workstreams/'))).toHaveLength(2);
    expect(requestedLlmInputs.at(-1)?.deliveryContract).toMatchObject({ kind: 'final_mission_package' });
    expect(requestedLlmInputs.at(-1)?.workflowPortfolio).toHaveLength(2);
    expect((await store.listDeliverables(mission.id))).toEqual(expect.arrayContaining([
      expect.objectContaining({
        uri: 'https://delivery-test.pinme.dev',
        mimeType: 'text/html',
        ipfsEvidence: expect.objectContaining({ provider: 'pinme_ipfs', versionNo: 1 }),
      }),
    ]));
    expect((await store.listDeliverables(mission.id))).toHaveLength(2);
    expect((await store.listDeliverables(mission.id)).at(-1)?.name).toContain('完整任务成果包');
    expect((await store.getMission(mission.id))?.status).toBe('review');
  });

  it('fails an official node instead of publishing a deterministic fallback as a deliverable', async () => {
    const officialOwner: UserContext = {
      id: 'agentmesh-official', displayName: 'AgentMesh Official', role: 'developer',
    };
    store.profiles.set(officialOwner.id, officialOwner);
    store.agents.set('official-delivery-writer', {
      ...agent('official-delivery-writer', '内容生成', 12),
      ownerId: officialOwner.id,
      name: 'Delivery Writer',
      endpoint: 'agentmesh://builtin/writing',
      official: true,
    });
    const { mission, stages } = await createMission();
    const assignments = Object.fromEntries(stages.map((stage) => [stage.id, 'official-delivery-writer']));
    expect((await api(`/api/missions/${mission.id}/workflow`, {
      method: 'POST', body: JSON.stringify({ assignments }),
    }, requester.id)).response.status).toBe(200);
    expect((await api(`/api/missions/${mission.id}/start`, {
    ...
    injectedLlmContent = 'not-json';

    const dispatched = await api(`/api/missions/${mission.id}/dispatch`, {
      method: 'POST', body: '{}',
    }, requester.id);

    expect(dispatched.response.status).toBe(503);
    expect(dispatched.body.error.code).toBe('BUILTIN_AGENT_MODEL_UNAVAILABLE');
    expect((await store.listStages(mission.id))[0]).toMatchObject({
      status: 'failed',
      output: { retryable: true, llmError: 'invalid_json' },
    });
    expect(await store.listDeliverables(mission.id)).toEqual([]);
  ...
  it('runs the deliverable, review, acceptance, and settlement lifecycle', async () => {
    ...
    ipfsResponder = () => new Response('gateway unavailable', { status: 503 });
  ...
  it('lets an admin manually pass a Trial without fabricating Endpoint health', async () => {
    const input = {
      score: 7.5,
      reason: 'Manual launch approval while the Agent upgrades its Trial protocol.',
    };
    const headers = { 'Idempotency-Key': 'admin-trial-override-1' };
    const forbidden = await api('/api/admin/agents/research-agent/quality/trial-override', {
      method: 'POST', headers: { 'Idempotency-Key': 'developer-trial-override-1' }, body: JSON.stringify(input),
    }, developer.id);
    const first = await api('/api/admin/agents/research-agent/quality/trial-override', {
      method: 'POST', headers, body: JSON.stringify(input),
    }, admin.id);
    const replay = await api('/api/admin/agents/research-agent/quality/trial-override', {
      method: 'POST', headers, body: JSON.stringify(input),
    }, admin.id);

    expect(forbidden.response.status).toBe(403);
    expect(first.response.status, JSON.stringify(first.body)).toBe(201);
    expect(first.body.data.trial).toMatchObject({
      suiteVersion: 'agentmesh.trial.manual.v1', status: 'passed', score: 75,
      evidence: { manualOverride: true, liveChallenge: false },
    });
    expect(first.body.data.agent).toMatchObject({ trustScore: 7.5, status: 'active' });
    expect(first.body.data.stats).toMatchObject({ trialPassed: true, endpointHealthy: false });
    expect(replay.body.meta.replayed).toBe(true);
    expect((await store.listAgentTrials('research-agent')).filter((trial) => trial.suiteVersion === 'agentmesh.trial.manual.v1')).toHaveLength(1);
    expect((await store.listAgentMetricEvents('research-agent')).filter((event) => event.type === 'trial_passed' && event.sourceType === 'admin')).toHaveLength(1);
  ...
  it('pauses dispatch at the store gate, resumes idempotently, and persists checkpoints', async () => {
    const { mission, stages } = await createFundedMission();
    const pauseHeaders = { 'Idempotency-Key': 'pause-runtime-0001' };
    const paused = await api(`/api/missions/${mission.id}/pause`, {
      method: 'POST', headers: pauseHeaders, body: JSON.stringify({ reason: '等待任务方确认外部资料后再继续。' }),
    }, requester.id);
    expect(paused.response.status).toBe(200);
    expect(paused.body.data.mission).toMatchObject({ status: 'paused', pauseMode: 'requester', schedulerRevision: 1 });
    expect(paused.body.data.checkpoints).toContainEqual(expect.objectContaining({ kind: 'pause', schedulerState: 'clean' }));

    const replay = await api(`/api/missions/${mission.id}/pause`, {
      method: 'POST', headers: pauseHeaders, body: JSON.stringify({ reason: '等待任务方确认外部资料后再继续。' }),
    }, requester.id);
    expect(replay.response.status).toBe(200);
    expect(replay.body.meta.replayed).toBe(true);

    const blocked = await api(`/api/missions/${mission.id}/dispatch`, { method: 'POST', body: '{}' }, requester.id);
    expect(blocked.response.status).toBe(409);
    expect(dispatchCalls).toBe(0);
    expect(await store.transitionStage(mission.id, stages[0].id, 'queued', 'running')).toBeNull();

    const resumed = await api(`/api/missions/${mission.id}/resume`, {
      method: 'POST', headers: { 'Idempotency-Key': 'resume-runtime-0001' }, body: '{}',
    }, requester.id);
    expect(resumed.response.status).toBe(200);
    expect(resumed.body.data.mission).toMatchObject({ status: 'running', pauseMode: null, schedulerRevision: 2 });
    expect((await store.listDirtyMissionControls()).some((item) => item.missionId === mission.id)).toBe(false);
    expect((await store.listWorkflowCheckpoints(mission.id)).map((item) => item.kind)).toEqual(expect.arrayContaining(['pause', 'resume', 'reconciled']));
    expect(await store.transitionStage(mission.id, stages[0].id, 'queued', 'running')).toMatchObject({ status: 'running' });
    expect(await store.transitionStage(mission.id, stages[0].id, 'queued', 'running')).toBeNull();
  });

  it('skips a false terminal condition Gate with a replayable transition checkpoint', async () => {
    const { mission, stages } = await createFundedMission();
    await completePausedWorkflow(mission.id, stages);
    const gate: WorkflowStage = {
      id: 'STAGE-conditional-gate', missionId: mission.id, position: stages.length + 1,
      nodeType: 'approval', positionX: 1_200, positionY: 200, progress: 0,
      name: '高风险复核', purpose: '仅在输出标记为高风险时审批。', category: '人工审批',
      budget: 0, status: 'queued', agentId: null, input: { approvalCriteria: '高风险证据必须人工确认。' },
      output: null, attemptNo: 1, attemptCreatedAt: currentNow, createdAt: currentNow, updatedAt: currentNow,
    };
    store.stages.set(mission.id, [...(store.stages.get(mission.id) ?? []), gate]);
    store.edges.set(mission.id, [...(store.edges.get(mission.id) ?? []), {
      id: 'EDGE-conditional-gate', missionId: mission.id, sourceStageId: stages.at(-1)!.id, targetStageId: gate.id,
      condition: { op: 'eq', path: '/risk', value: 'high' }, createdAt: currentNow,
    }]);

    const runningMission = await store.getMission(mission.id);
    if (!runningMission) throw new Error('mission should exist');
    const first = await resolveWorkflowTransitions(store, runningMission, currentNow);
    const second = await resolveWorkflowTransitions(store, runningMission, currentNow);

    expect(first.stages.find((stage) => stage.id === gate.id)).toMatchObject({
      status: 'done', output: { skipped: true, verified: true },
    });
    expect(second.transitions.filter((checkpoint) => checkpoint.edgeId === 'EDGE-conditional-gate')).toHaveLength(1);
    expect(second.transitions).toContainEqual(expect.objectContaining({ matched: false, sourceAttemptNo: 1, errorCode: null }));
    expect((await store.listEvents(mission.id)).filter((event) => event.type === 'gate.condition_skipped')).toHaveLength(1);
  });

  it('fails a condition Gate instead of skipping it when the resolved operand is non-scalar', async () => {
    const { mission, stages } = await createFundedMission();
    await completePausedWorkflow(mission.id, stages);
    await store.updateStage(mission.id, stages.at(-1)!.id, 'done', {
      summary: 'Final output', risk: { level: 'high' }, verified: true,
    });
    const gate: WorkflowStage = {
      id: 'STAGE-invalid-condition-gate', missionId: mission.id, position: stages.length + 1,
      nodeType: 'approval', positionX: 1_200, positionY: 200, progress: 0,
      name: '风险复核', purpose: '复核高风险输出。', category: '人工审批', budget: 0,
      status: 'queued', agentId: null, input: { approvalCriteria: '风险必须为标量。' }, output: null,
      attemptNo: 1, attemptCreatedAt: currentNow, createdAt: currentNow, updatedAt: currentNow,
    };
    store.stages.set(mission.id, [...(store.stages.get(mission.id) ?? []), gate]);
    store.edges.set(mission.id, [...(store.edges.get(mission.id) ?? []), {
      id: 'EDGE-invalid-condition-gate', missionId: mission.id,
      sourceStageId: stages.at(-1)!.id, targetStageId: gate.id,
      condition: { op: 'eq', path: '/risk', value: 'high' }, createdAt: currentNow,
    }]);
    const runningMission = await store.getMission(mission.id);
    if (!runningMission) throw new Error('mission should exist');

    const resolved = await resolveWorkflowTransitions(store, runningMission, currentNow);

    expect(resolved.stages.find((stage) => stage.id === gate.id)).toMatchObject({
      status: 'failed', output: { error: { code: 'INVALID_CONDITION_OPERAND' } },
    });
    expect(resolved.transitions).toContainEqual(expect.objectContaining({
      edgeId: 'EDGE-invalid-condition-gate', matched: false, errorCode: 'INVALID_CONDITION_OPERAND',
    }));
    expect((await store.listEvents(mission.id)).filter((event) => event.type === 'gate.condition_skipped')).toHaveLength(0);
  });

  it('rejects an isolated retry when an immutable upstream mapping checkpoint caused the failure', async () => {
    const { mission, stages } = await createFundedMission();
    const [source, target] = stages;
    store.stages.set(mission.id, stages.map((stage) => {
      if (stage.id === source.id) return { ...stage, status: 'done', progress: 100, output: { verified: true } };
      if (stage.id === target.id) return { ...stage, status: 'queued', progress: 0, output: null };
      return stage;
    }));
    store.edges.set(mission.id, [{
      id: 'EDGE-required-mapping', missionId: mission.id, sourceStageId: source.id, targetStageId: target.id,
      mappings: [{ from: '/summary', to: '/upstreamSummary', required: true }], createdAt: currentNow,
    }]);
    const runningMission = await store.getMission(mission.id);
    if (!runningMission) throw new Error('mission should exist');
    await resolveWorkflowTransitions(store, runningMission, currentNow);
    expect((await store.listStages(mission.id)).find((stage) => stage.id === target.id)).toMatchObject({
      status: 'failed', output: { error: { code: 'MAPPING_REQUIRED_SOURCE_MISSING' } }, attemptNo: 1,
    });

    const retry = await api(`/api/missions/${mission.id}/nodes/${target.id}/retry`, { method: 'POST', body: '{}' }, requester.id);

    expect(retry.response.status).toBe(409);
    expect(retry.body.error).toMatchObject({
      code: 'MAPPING_SOURCE_REWORK_REQUIRED',
      details: { sourceStageIds: [source.id], errorCodes: ['MAPPING_REQUIRED_SOURCE_MISSING'] },
    });
    expect((await store.listStages(mission.id)).find((stage) => stage.id === target.id)).toMatchObject({ status: 'failed', attemptNo: 1 });
    expect((await store.getMission(mission.id))?.status).toBe('running');
  });

  it('replays an immutable Memory checkpoint after its source advances to a newer attempt', async () => {
    const { mission, stages } = await createFundedMission();
    const edge = (await store.listEdges(mission.id))[0];
    await store.updateStage(mission.id, edge.sourceStageId, 'done', { summary: 'Attempt one complete', verified: true });
    const checkpoint = {
      id: 'TRANSITION-MEMORY-REPLAY', missionId: mission.id, edgeId: edge.id,
      sourceStageId: edge.sourceStageId, targetStageId: edge.targetStageId,
      sourceAttemptNo: 1, workflowVersion: mission.workflowVersion, matched: true,
      mappedInput: {}, missingRequired: [], errorCode: null, createdAt: currentNow,
    };
    expect(await store.recordWorkflowTransition(checkpoint)).toMatchObject({ applied: true });
    store.stages.set(mission.id, stages.map((stage) => stage.id === edge.sourceStageId
      ? { ...stage, status: 'queued', attemptNo: 2, attemptCreatedAt: currentNow }
      : stage));

    expect(await store.recordWorkflowTransition({ ...checkpoint, id: 'TRANSITION-MEMORY-LATE' })).toMatchObject({
      applied: false,
      checkpoint: { id: checkpoint.id },
    });
    expect(await store.listCurrentWorkflowTransitions(mission.id)).toEqual([]);
    expect(await store.listWorkflowTransitions(mission.id)).toEqual([checkpoint]);
  });

  it('returns complete transition history beyond the old scheduler scan limit', async () => {
    const checkpoints = Array.from({ length: 5_001 }, (_, index): WorkflowTransitionCheckpoint => ({
      id: `TRANSITION-HISTORY-${index}`, missionId: 'MISSION-LONG-HISTORY', edgeId: `EDGE-${index}`,
      sourceStageId: 'STAGE-source', targetStageId: 'STAGE-target', sourceAttemptNo: index + 1,
      workflowVersion: 1, matched: true, mappedInput: {}, missingRequired: [], errorCode: null,
      createdAt: new Date(Date.UTC(2026, 7, 16, 0, 0, index)).toISOString(),
    }));
    store.workflowTransitions.set('MISSION-LONG-HISTORY', checkpoints);

    expect(await store.listWorkflowTransitions('MISSION-LONG-HISTORY')).toHaveLength(5_001);
    expect(await store.listWorkflowTransitions('MISSION-LONG-HISTORY', 25)).toHaveLength(25);
  });

  it('rejects an oversized merged mapping before claiming the stage or charging Agent quality', async () => {
    const { mission, stages } = await createFundedMission();
    const [left, right, target] = stages;
    store.stages.set(mission.id, stages.map((stage) => stage.id === target.id
      ? { ...stage, status: 'queued' }
      : { ...stage, status: 'done', progress: 100, output: { summary: `${stage.name} complete`, verified: true } }));
    const edges: WorkflowEdge[] = [
      {
        id: 'EDGE-MEMORY-LARGE-LEFT', missionId: mission.id,
        sourceStageId: left.id, targetStageId: target.id,
        mappings: [{ from: '/large', to: '/left', required: true }], createdAt: currentNow,
      },
      {
        id: 'EDGE-MEMORY-LARGE-RIGHT', missionId: mission.id,
        sourceStageId: right.id, targetStageId: target.id,
        mappings: [{ from: '/large', to: '/right', required: true }], createdAt: currentNow,
      },
    ];
    store.edges.set(mission.id, edges);
    for (const [index, edge] of edges.entries()) {
      await store.recordWorkflowTransition({
        id: `TRANSITION-MEMORY-LARGE-${index}`, missionId: mission.id, edgeId: edge.id,
        sourceStageId: edge.sourceStageId, targetStageId: target.id, sourceAttemptNo: 1,
        workflowVersion: mission.workflowVersion, matched: true,
        mappedInput: { [index ? 'right' : 'left']: 'x'.repeat(9_000) },
        missingRequired: [], errorCode: null, createdAt: currentNow,
      });
    }
    const targetAgent = store.agents.get(target.agentId!);
    const qualityBefore = targetAgent ? { jobs: targetAgent.jobs, successRate: targetAgent.successRate } : null;

    const response = await api(`/api/missions/${mission.id}/dispatch`, { method: 'POST', body: '{}' }, requester.id);

    expect(response.response.status).toBe(413);
    expect(response.body.error.code).toBe('MAPPED_INPUT_TOO_LARGE');
    expect((await store.listStages(mission.id)).find((stage) => stage.id === target.id)).toMatchObject({ status: 'queued' });
    expect(dispatchCalls).toBe(0);
    expect(store.agents.get(target.agentId!)).toMatchObject(qualityBefore ?? {});
  });

  it('keeps a newer same-mode pause when an older resume arrives late', async () => {
    const { mission } = await createFundedMission();
    const firstPause = await store.pauseMission(
      mission.id, requester.id, 'requester', '第一次暂停等待确认。', '2026-08-16T00:01:00.000Z',
    );
    ...
    expect((await store.resumeMission(
      mission.id, requester.id, 'requester', firstPause.schedulerRevision, '2026-08-16T00:02:00.000Z',
    )).state).toBe('applied');
    expect((await store.pauseMission(
      mission.id, requester.id, 'requester', '发现新证据后再次暂停。', '2026-08-16T00:03:00.000Z',
    )).state).toBe('applied');

    expect((await store.resumeMission(
      mission.id, requester.id, 'requester', firstPause.schedulerRevision, '2026-08-16T00:04:00.000Z',
    )).state).toBe('invalid');
    expect(await store.getMission(mission.id)).toMatchObject({
      status: 'paused', pauseReason: '发现新证据后再次暂停。',
    });
  });

  it('preserves start replay and returns review after a completed paused workflow resumes', async () => {
    const { mission, stages } = await createFundedMission();
    const paused = await api(`/api/missions/${mission.id}/pause`, {
      method: 'POST', body: JSON.stringify({ reason: '等待最后一批在途结果。' }),
    }, requester.id);
    expect(paused.response.status).toBe(200);

    const startReplay = await api(`/api/missions/${mission.id}/start`, { method: 'POST', body: '{}' }, requester.id);
    expect(startReplay.response.status).toBe(200);
    expect(startReplay.body).toMatchObject({ data: { mission: { status: 'paused' } }, meta: { replayed: true } });

    await completePausedWorkflow(mission.id, stages);
    const resumed = await api(`/api/missions/${mission.id}/resume`, { method: 'POST', body: '{}' }, requester.id);
    expect(resumed.response.status).toBe(200);
    expect(resumed.body.data.mission).toMatchObject({ status: 'review', progress: 100 });
    expect((await store.listDirtyMissionControls()).some((item) => item.missionId === mission.id)).toBe(false);
  });

  it('uses full readiness reconciliation when cron recovers a dirty resume checkpoint', async () => {
    const { mission, stages } = await createFundedMission();
    const paused = await store.pauseMission(
      mission.id, requester.id, 'requester', '模拟恢复请求在对账前崩溃。', '2026-08-16T00:01:00.000Z',
    );
    ...
    const resumed = await store.resumeMission(
      mission.id, requester.id, 'requester', paused.schedulerRevision, '2026-08-16T00:06:00.000Z',
    );
    expect(resumed.state).toBe('applied');
    expect((await store.listDirtyMissionControls()).some((item) => item.missionId === mission.id)).toBe(true);

    currentNow = '2026-08-16T00:07:00.000Z';
    const waits: Promise<unknown>[] = [];
    await app.scheduled(null, testEnv, {
      waitUntil(promise: Promise<unknown>) { waits.push(Promise.resolve(promise)); },
    });
    await Promise.all(waits);

    expect(await store.getMission(mission.id)).toMatchObject({ status: 'review', progress: 100 });
    expect((await store.listDirtyMissionControls()).some((item) => item.missionId === mission.id)).toBe(false);
    expect((await store.listWorkflowCheckpoints(mission.id))[0]).toMatchObject({ kind: 'reconciled', schedulerState: 'clean' });
  });

  it('cleans a dirty checkpoint after review committed before reconciliation', async () => {
    const { mission } = await createFundedMission();
    const paused = await store.pauseMission(
      mission.id, requester.id, 'requester', '模拟提交验收前的恢复检查点。', '2026-08-16T00:01:00.000Z',
    );
    expect(paused.state).toBe('applied');
    if (paused.state !== 'applied') throw new Error('pause should apply');
    const resumed = await store.resumeMission(
      mission.id, requester.id, 'requester', paused.schedulerRevision, '2026-08-16T00:02:00.000Z',
    );
    expect(resumed.state).toBe('applied');
    await store.submitMissionForReview(mission.id, '2026-08-23T00:02:00.000Z');
    expect(await store.getMission(mission.id)).toMatchObject({ status: 'review' });
    expect(await store.listDirtyMissionControls()).toContainEqual({
      missionId: mission.id, schedulerRevision: resumed.state === 'applied' ? resumed.schedulerRevision : -1,
    });
  ...
  it('returns the reconciled aggregate after review rework starts a new attempt', async () => {
    const { mission, stages } = await createFundedMission();
    await completePausedWorkflow(mission.id, stages);
    const review = await api(`/api/missions/${mission.id}/review`, { method: 'POST', body: '{}' }, developer.id);
    expect(review.response.status).toBe(200);
    expect(review.body.data).toMatchObject({ status: 'review', progress: 100 });

    const target = stages.find((stage) => stage.nodeType === 'task')!;
    const headers = { 'Idempotency-Key': 'review-rework-replay-0001' };
    const body = JSON.stringify({
      targetStageIds: [target.id],
      reason: '验收发现当前版本仍缺少一项关键约束说明。',
      acceptanceCriteria: '新 attempt 必须补齐约束说明并重新生成可验证证据。',
    });
    const changed = await api(`/api/missions/${mission.id}/change-requests`, {
      method: 'POST',
      headers,
      body,
    }, requester.id);
    ...
    expect(changed.body.data.mission).toMatchObject({
      status: persisted?.status,
      progress: persisted?.progress,
      currentStage: persisted?.currentStage,
      schedulerRevision: persisted?.schedulerRevision,
    });
    expect(changed.body.data.mission.progress).toBeLessThan(100);

    const replay = await api(`/api/missions/${mission.id}/change-requests`, {
      method: 'POST', headers, body,
    }, requester.id);
    expect(replay.response.status).toBe(201);
    expect(replay.body.meta.replayed).toBe(true);
    expect(replay.body.data.changeRequests).toHaveLength(1);
  });

  it('allows an admin mission owner to request review rework', async () => {
    const { mission, stages } = await createFundedMission();
    await completePausedWorkflow(mission.id, stages);
    expect((await api(`/api/missions/${mission.id}/review`, {
      method: 'POST', body: '{}',
    }, developer.id)).response.status).toBe(200);
    store.profiles.set(requester.id, { ...requester, role: 'admin' });
    const target = stages.find((stage) => stage.nodeType === 'task')!;
    ...
    const changed = await api(`/api/missions/${mission.id}/change-requests`, {
      ...
      body: JSON.stringify({
        targetStageIds: [target.id],
        reason: '结构化输出字段类型不符合验收契约，需要重新执行。',
        acceptanceCriteria: 'summary 为字符串、findings 为字符串数组，并保留验证建议。',
      }),
    }, requester.id);

    expect(changed.response.status).toBe(201);
    expect(changed.body.data.stages).toContainEqual(expect.objectContaining({
      id: target.id,
      status: 'queued',
      attemptNo: 2,
    }));
  ...
  it('reports a conflict without notifying when pause wins the manual review race', async () => {
    const { mission, stages } = await createFundedMission();
    await completePausedWorkflow(mission.id, stages);
    const submitForReview = store.submitMissionForReview.bind(store);
    vi.spyOn(store, 'submitMissionForReview').mockImplementationOnce(async (missionId, dueAt) => {
      const paused = await store.pauseMission(
        missionId, requester.id, 'requester', '任务方在验收切换前要求暂停。', currentNow,
      );
      expect(paused.state).toBe('applied');
      return submitForReview(missionId, dueAt);
    });

    const review = await api(`/api/missions/${mission.id}/review`, { method: 'POST', body: '{}' }, developer.id);
    expect(review.response.status).toBe(409);
    expect(review.body.error.code).toBe('MISSION_REVIEW_CONFLICT');
    expect(await store.getMission(mission.id)).toMatchObject({ status: 'paused' });
    expect((await store.listNotifications(requester.id)).filter((item) => item.title === '任务已进入验收')).toHaveLength(0);
  });

  it('enforces requester and emergency pause permissions', async () => {
    const { mission } = await createFundedMission();
    const developerPause = await api(`/api/missions/${mission.id}/pause`, {
      method: 'POST', body: JSON.stringify({ reason: '开发者不应能够暂停任务方的任务。' }),
    }, developer.id);
    expect(developerPause.response.status).toBe(403);

    const requesterPause = await api(`/api/missions/${mission.id}/pause`, {
      method: 'POST', body: JSON.stringify({ reason: '任务方先暂停以核对外部资料。' }),
    }, requester.id);
    expect(requesterPause.response.status).toBe(200);
    expect(requesterPause.body.data.mission).toMatchObject({ status: 'paused', pauseMode: 'requester', schedulerRevision: 1 });

    const emergency = await api(`/api/missions/${mission.id}/pause`, {
      method: 'POST', body: JSON.stringify({ reason: '检测到需要人工核验的平台级风险。' }),
    }, admin.id);
    expect(emergency.response.status).toBe(200);
    expect(emergency.body.data.mission).toMatchObject({
      status: 'paused', pauseMode: 'emergency', schedulerRevision: 2,
      pauseReason: '检测到需要人工核验的平台级风险。', pausedBy: admin.id,
    });

    const requesterResume = await api(`/api/missions/${mission.id}/resume`, { method: 'POST', body: '{}' }, requester.id);
    expect(requesterResume.response.status).toBe(403);
    expect(requesterResume.body.error.code).toBe('EMERGENCY_PAUSE_ADMIN_REQUIRED');
    expect((await api(`/api/missions/${mission.id}/resume`, { method: 'POST', body: '{}' }, admin.id)).response.status).toBe(200);
  });

  it('keeps assistance and dispute controls available during an emergency pause', async () => {
    const { mission } = await createFundedMission();
    const emergency = await api(`/api/missions/${mission.id}/pause`, {
      method: 'POST', body: JSON.stringify({ reason: '检测到交付风险，需要暂停调度并保留人工处置入口。' }),
    }, admin.id);
    expect(emergency.response.status).toBe(200);
    expect(emergency.body.data.mission.status).toBe('paused');

    const assistance = await api(`/api/missions/${mission.id}/events`, {
      method: 'POST', body: JSON.stringify({ type: 'mission.assistance_requested' }),
    }, requester.id);
    expect(assistance.response.status).toBe(201);
    expect(assistance.body.data.type).toBe('mission.assistance_requested');

    const dispute = await api(`/api/missions/${mission.id}/disputes`, {
      method: 'POST',
      body: JSON.stringify({ reason: '暂停期间发现交付证据存在重大矛盾，需要冻结托管并由平台介入核查。', evidence: [] }),
    }, requester.id);
    expect(dispute.response.status).toBe(201);
    expect(dispute.body.data).toMatchObject({ missionId: mission.id, status: 'open' });
    expect(await store.getEscrow(mission.id)).toMatchObject({ status: 'frozen' });
  });

  it('creates append-only rework attempts while preserving prior output', async () => {
    const { mission, stages } = await createFundedMission();
    const target = stages[0];
    const oldOutbox = [...store.dispatchOutbox.values()].find((item) => item.missionId === mission.id && item.stageId === target.id);
    expect(oldOutbox).toBeDefined();
    const initialDispatch = await api(`/api/missions/${mission.id}/dispatch`, { method: 'POST', body: '{}' }, requester.id);
    ...
    const changed = await api(`/api/missions/${mission.id}/change-requests`, {
      ...
      body: JSON.stringify({
        targetStageIds: [target.id],
        reason: '新证据使原始结论需要重新核验。',
        acceptanceCriteria: '新版本必须逐条引用证据并说明与旧版的差异。',
      }),
    }, requester.id);
    expect(changed.response.status).toBe(201);
    expect(changed.body.data.changeRequests[0]).toMatchObject({
      version: 1, targetStageIds: [target.id],
      priorStageState: expect.arrayContaining([expect.objectContaining({
        stageId: target.id, attemptNo: 1, input: target.input,
        output: { summary: '原始交付', evidence: ['v1'] },
      })]),
    });
    expect(changed.body.data.stages.find((stage: JsonBody) => stage.id === target.id)).toMatchObject({
      status: 'queued', attemptNo: 2, output: null,
      input: { rework: {
        reason: '新证据使原始结论需要重新核验。',
        acceptanceCriteria: '新版本必须逐条引用证据并说明与旧版的差异。',
        targetStageIds: [target.id], isTarget: true,
      } },
    });
    expect(changed.body.data.mission).toMatchObject({ status: 'paused' });
    expect(store.dispatchOutbox.get(oldOutbox!.id)?.status).toBe('done');

    const staleCallback = await api(`/api/hooks/agents/${oldDispatchBody.task.agent.id}/events`, {
      method: 'POST',
      headers: { 'X-AgentMesh-Signature': oldDispatchBody.callback.signature },
      body: JSON.stringify({
        missionId: mission.id, stageId: target.id, runId: oldDispatchBody.callback.runId,
        callbackId: 'callback-stale-invalid-delivery', expiresAt: oldDispatchBody.callback.expiresAt,
        status: 'done', output: {},
      }),
    });
    expect(staleCallback.response.status).toBe(409);
    expect(staleCallback.body.error.code).toBe('INVALID_STAGE_TRANSITION');
    expect((await store.listAgentMetricEvents(oldDispatchBody.task.agent.id)).filter((event) => (
      event.type === 'artifact_invalid' && event.detail.runId === oldDispatchBody.callback.runId
    ))).toHaveLength(0);

    expect((await api(`/api/missions/${mission.id}/resume`, { method: 'POST', body: '{}' }, requester.id)).response.status).toBe(200);
    const rotatedOutbox = store.dispatchOutbox.get(oldOutbox!.id);
    expect(rotatedOutbox).toMatchObject({ status: 'pending' });
    expect(rotatedOutbox?.runId).not.toBe(oldOutbox!.runId);

    dispatchedBody = null;
    const dispatched = await api(`/api/missions/${mission.id}/dispatch`, { method: 'POST', body: '{}' }, requester.id);
    expect(dispatched.response.status).toBe(202);
    expect(dispatchedBody?.task.node.input.rework).toMatchObject({
      reason: '新证据使原始结论需要重新核验。',
      acceptanceCriteria: '新版本必须逐条引用证据并说明与旧版的差异。',
      targetStageIds: [target.id], isTarget: true,
    });
