...
async function completePausedWorkflow(missionId: string, stages: WorkflowStage[]) {
  ...
  for (const stage of stages) {
    ...
    const deliverable: Deliverable = {
      ...
      contentHash: `sha256:${'c'.repeat(64)}`, mimeType: 'application/json', status: 'submitted', createdAt: completedAt,
    };
...
describe('AgentMesh Worker', () => {
  ...
  it('auto-accepts and completes stages assigned to runnable official test Agents', async () => {
    ...
    expect(callbackSpy.mock.calls.every(([update]) => update.artifacts?.length === 1)).toBe(true);
